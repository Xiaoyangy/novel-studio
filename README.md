...
`project-all` 推演期间即使暂时没有新事件，也会持续显示当前推演章节；过期租约
或已退出进程不会冒充运行中。页面若仍停留在旧接口缓存，重新打开根地址即可。
