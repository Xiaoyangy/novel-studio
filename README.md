...
- Project-Arc 的 world simulation 不再把一次长会话当成唯一成败边界：首次会话上限收紧为 12 turns，每次成功工具调用继续落到同一 non-canon partial；会话预算耗尽后，宿主读取精确 gaps，用最多三次有界 fresh session 续接，恢复会话最多 6 turns。上一会话最近的 `simulate_chapter_world` 校验错误会经过解码、去重和限长后随 gaps 一起进入下一恢复提示，避免模型只知道“还缺什么”却不知道“上次为何被拒”；无新工具调用的会话不会擦掉上一条有效诊断。若最后一轮刚好补齐字段却没来得及 finalize，宿主会用当前 execution lease 刷新 `novel_context` access receipt 并原子收口，不要求人工重跑，也不复用旧 PID 的 token。
