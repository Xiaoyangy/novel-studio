...
- 进度看板新增独立 RAG 运行态与正式弧规划口径：冻结的 420 章分层大纲不再冒充第一弧 12 章已正式推演，building bundle、seal receipt 与正文章号各自显示；章零规划和 RAG 重建均不再误报“正在执行第 1 章”。
...
└── output/novel/
    ...
    └── meta/
        ...
        ├── pipeline.json
        ├── checkpoints.jsonl
        ├── planning/outline_repair/       # host-only operation 0 intent/receipt（使用定向修复时）
        ├── architect_readiness.*
