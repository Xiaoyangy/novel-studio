...
[为什么是 novel-studio](#为什么是-novel-studio) · [效果预览](#效果预览) · [快速开始](#快速开始) · [工作流](#从世界到正文) · [渲染风格](#渲染风格只改变怎么写) · [RAG](#rag-不是装饰) · [文档](#文档与社区)
...
| 📖 读者优先渲染 | 三采样选稿与正文评分以可读性为先：现场是否具体、对白是否活、节奏是否有起伏、主视角是否在场、章末是否有前推力；反 AI 腔与检测器只作护栏，不作目标 |
| ✍️ 可审计风格渲染 | 配置风格与已验收正文的防复读记忆合成为不可变合同；Drafter 与 Editor 消费同一份由摘要绑定的不可变合同，风格只能改变表达，不能改写剧情事实 |
...
重复同一条命令会按落盘证据幂等继续。身份完整的候选稿可以接着补齐审核与发布回执，不会仅因进程崩溃再次调用 Drafter；摘要漂移、证据缺失或路径异常会在 provider 调用前失败闭锁。
...
当前协议要求六项正式审核工件共同绑定 exact body：Editor JSON、可读报告、机械 AI gate、AI 声纹红旗、DeepSeek judge 与模型来源证明。acceptance 回执中的正式路径集合出现缺项、替换或额外项，或者正文发生漂移，都会阻止验收。
...
| `novel-studio --pipeline --stages render --refresh-render-input --dir ...` | 仅为尚未开始且没有 durable candidate evidence 的 sealed 章节，显式刷新 model / provider / prompt 绑定 |
