...
[![Platform](https://img.shields.io/badge/macOS%20%7C%20Linux-supported-555)](https://github.com/Xiaoyangy/novel-studio/releases/latest)
...
novel-studio 是一个开源、自托管、local-first 的 AI 小说生产系统，面向 2—3 万字短篇、百万字连载、长篇网文和整书工程化创作。
...
- 零号资产按项目题材、角色和冻结章纲生成，不从引擎源码注入任何样书专名或类型模板；关系分类要求成对、定向且有明确证据，角色只能在全书章纲规定的首登场章及以后进入 world tick。人物 dossier、known facts、forbidden knowledge 和首章上下文只保留当时可知事实，未来弧职责、系统秘密、后续关系与作者说明不能提前写入角色记忆或 RAG。
...
`finalize` 读取全部 canonical accepted finals，生成全局 exact-book review、`正文.md`、`meta/finalization.*` 与 `meta/publication_package.*`；`deliver` 只在这些工件与当前章节哈希一致时生成交付沉淀。
...
flowchart LR
    ...
    ACR -->|尚有下一弧| PP
    ACR -->|全书完成| FI["Finalize<br/>全文终审 + 正文.md"]
...
| `finalize` | 全局 exact-book review、确定性合并的 `正文.md`、finalization 与 publication package | 只在全部章节和 terminal arc receipt 齐全后显式执行；global reject 不制造完结工件 |
...
flowchart TD
    ...
    ARC -->|terminal arc| FI["Explicit finalize<br/>global exact-book review"]
...
| `novel-studio --pipeline --stages finalize,deliver` | 全书逐章通过后执行全文终审，生成 `正文.md` 与出版包并完成交付校验 |
