...
| 流水线 | `meta/pipeline.json` | `completed` 阶段的 `evidence` 是否仍指向非空产物/checkpoint | 已完成阶段均为 `verified` 且证据存在 | 证据缺失/失效（PipelineEvidenceDrift 命中） |
| Coordinator 会话 | `meta/sessions/coordinator.jsonl` | 最近 5-10 轮的 tool_call 模式 | 单轮快速推进 | 同一工具空调多次（卡死循环） |
| 世界推演游标 | `meta/world_tick.json` | `through_chapter` 与最新完成章差距 | gap ≤ 15 章（每弧边界一 tick） | gap > 15 章（OffscreenWorldStale 命中：镜头外世界停摆） |
| 离屏事件流 | `meta/world_events.jsonl` | `visibility_chapter` 分布 | 近期与远期事件混合（有埋线余量） | 全部已浮出（伏笔素材断供）或 actor 大量不在册 |
| 离屏日程账本 | `meta/offscreen_agenda.json` | active 日程的 `last_advanced_chapter` | 每次 tick 后刷新 | active 日程停滞 > 15 章（角色"假装有事做"） |
...
`diag.Snapshot.CastLedger` 会被 `internal/diag/rules_context.go` 的三条规则消费：

- `CastBriefRoleMissing`：反复出场（≥2 次）但缺少 `brief_role` 的配角会提示；3 个以上升级为 warning。
- `CastBloat`：已完成 ≥10 章后，未升格配角数超过 `已完成章数 × 0.8`（最低阈值 8）会提示；超过 1.2 倍升级为 warning。
- `CastPromotionCandidate`：`appearance_count ≥ 5` 且未 `promoted` 的配角会建议升格到 `characters.json` 或在弧摘要中明确人物功能。

这些规则只做观测和建议，不自动改 `cast_ledger.json` 或 `characters.json`。如果 `/diag` 命中后确实是同一角色多名、过场群众入册或高频配角长期没人物弧，先修 Writer 的 `characters/cast_intros` 输出习惯，再考虑手工合并或升格。
...
`/diag` 会把近端 session 中的空参数工具调用聚合到 `diag-export.md`；连续或高频
`novel_context{}` 会触发 `EmptyNovelContextLoop`，用于自动捕捉 Coordinator /
Writer 在原地反复查上下文但状态未推进的情况。
...
- [ ] `/diag` 命中 `EmptyNovelContextLoop`（连续 ≥ 5 次或近端高频空调 `novel_context{}`）
