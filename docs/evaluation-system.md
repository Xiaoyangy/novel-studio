...
internal/eval/
  ...
  inspect.go     离线检查既有 output/novel 产物，不启动模型生成
  runner.go      装配 host 驱动（按章数上限截停 + drain 到 Done），bundle.OverridePrompt 内存覆盖
  collect.go     对产出目录跑 diag.Diagnose + stylestat.Compute + usage/tool_calls + RAG/Qdrant + 契约断言
...
`eval inspect` 是独立的离线检查子命令，已实现参数为：`--cases`（必填）、`--dir`（覆盖 case 的 `artifact_dir`）、`--config`、`--out`。它只读取已有 `output/novel` 工件，不启动模型、不写新章节，适合在继续推进真实项目之前做 Harness 验收。

Harness case 可在 `expect` 中声明工程契约：

- `required_files`：必须存在的台账、章节、评审、摘要、RAG 状态文件。
- `min_rag_chunks` / `required_rag_facets` / `required_rag_source_kinds` / `required_rag_sources` / `forbidden_rag_sources`：约束 RAG 沉淀是否覆盖章节推进、人物变化、项目进度、时间线、世界规则、资源账本、写法资产等来源，并防止跨项目或参考库污染。
- `vector_store` / `embedding_provider` / `embedding_model` / `min_vector_points`：约束本地 embedding 与向量库配置。
- `require_qdrant` / `min_qdrant_points`：要求 Qdrant 可连通且集合点数达到阈值。若 Qdrant 没启动，Harness 会失败或告警，而不是静默跳过。
