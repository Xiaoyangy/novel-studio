...
2026-07-08 的工程交付文档把最新架构图、主流水线、审核门禁流程、DeepSeek 复审链路和《她的第二算法》第一章进度图放在一起，见 [engineering-delivery-20260708.md](engineering-delivery-20260708.md)。本文继续保留运行时架构原则与底层约束，交付页用于对外快速浏览。
...
`Phase` 默认只允许同态或向前迁移；`complete -> writing` 只能通过 `reopen_book` /
`ProgressStore.Reopen` 这个受 `phase=complete` 前置守卫保护的返工出口发生。普通
`StartChapter` 会校验状态迁移，不能静默重开已完结作品。
...
`--pipeline` 的 `meta/pipeline.json` 只是阶段级断点索引，不是完成事实源。阶段被写入
`completed` 前必须经过证据复核，并把本轮证据写入 `evidence`：`write` 需要
`progress.phase=complete`、非空章节文件和对应 `commit_chapter` checkpoint；`review`
需要机械门禁、AI voice、Editor JSON、异模型裸正文判定、统一报告和项目汇总全部绑定
当前章节 `body_sha256`；`rewrite` 只接受当前审核，并需要重写后的章节和
`.pre-rewrite.md` 备份；`deliver` 只接受当前版本 `verdict=accept` 并要求非空交付日志。
每个阶段还会记录 artifact SHA-256，最终阶段完成后统一对账，避免后续阶段的合法改写让
早期证据停留在中间态。重跑时若已完成阶段的证据失效，
pipeline 必须清掉该阶段完成标记并重跑，而不是盲目跳过。`diag` 会只读加载
`meta/pipeline.json`，报告已完成阶段的证据漂移、缺失证据和待重跑阶段；它只产出
Finding，不清标记、不补 checkpoint、不触发流水线。
...
| `check_consistency` | 当前 `drafts/chXX.draft.md` 指纹（checkpoint digest） | consistency_check |
...
`commit_chapter` 承担弧/卷/全书完成检测，返回 19 个事实字段（`arc_end` / `needs_expansion` / `book_complete` 等；启用机械规则检查时再附 `rule_violations`）。正常写作路径存在 `draft` checkpoint 后，commit 必须看到同一草稿指纹的 `consistency_check`，并硬校验正文标题与 `plan.title` 一致。机械门禁失败会写 `reviews/NN_ai_gate.json` 并入队返工，`novel_context` 会把轻量 `mechanical_gate` 摘要注入 `rewrite_brief`，让 Writer 能看到高风险维度、规则命中和返工重点。`save_review` 承担 verdict 升级（评分卡门禁、契约 missed → rewrite、critical issue → rewrite、error issue → polish）。这些过去散落在 policy 层的逻辑现在固化在工具内部。
...
职责：启动时按 30000 字阈值和题材结构选规划师 → 规划补齐循环 → 收 `[Host 下达指令]` 立即生成对应 `subagent` tool_call → 处理 `[用户干预]` 自主裁定 → `save_review` / `save_foundation` 返回 `book_complete=true` 后输出总结。短篇/三万字内项目由 `scope=global` 全文终审通过后落 `正文.md` 并完成；长篇由章审、弧卷评审/摘要和 `complete_book` 收尾。
...
`agentcore` 现在是 `go.mod` 中的外部模块依赖（`github.com/voocel/agentcore`），不是本仓库的 `go.work` 兄弟目录。新架构用到的原语通过这个模块接入：`Steer` / `Subscribe` / `WithMaxTurns` / `WithStopGuard` / `WithToolGate` / `SubAgentConfig` / `ContextManagerFactory`。
...
**UI 聚合**：`Snapshot()` —— 上层入口一次拉取所有展示数据（实现见 `internal/host/host_snapshot.go`）
...
github.com/voocel/agentcore  通用 Agent 框架模块（应用层只接入，不放小说业务）
github.com/voocel/litellm     LLM 网关模块
```
