...
需要逐章评审文件和 `meta/review-summary.md`；`rewrite` 需要重写后的章节和
`.pre-rewrite.md` 备份；`export` 需要非空导出文件。重跑时若已完成阶段的证据失效，
