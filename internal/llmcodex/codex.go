...
func buildProsePrompt(messages []agentcore.Message) string {
	...
	suffix := `## 现在写正文
你面对的是连载读者，不是计划审核人。只使用 render_packet、style_contract、literary_render_contract、人物关系、前章尾声和经净化的外审证据；不要猜测已隐藏的完整章纲、世界推演或旧稿。

literary_render_contract 是本章唯一的文学渲染合同，style_contract 只约束题材语域和人物边界。craft_cards 是可省略的候选手法；source_refs 只用于追溯，路径、ID和资料原句不得进入正文。
...
任何非人物媒介、界面或传话声音，只在合同明确存在时使用，只承担当下必需的一件事，不重讲已经看见的因果。若 draft_external_ai_review 为 blocking，只吸收其可读性证据，不沿旧稿段落顺序做同义改写，也不为追分制造随机缺口。
