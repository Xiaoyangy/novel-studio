...
type RuntimeCapture struct {
	...
	EmptyToolCalls  []EmptyToolStat
	DupContent      []DupStat // 同 sha 文本反复出现（反复生成同段）
	LogKinds        map[string]int
	LogErrors       int
	LogWarns        int
	StopGuard       int
	ContextRewrites []ContextRewriteStat
...
// EmptyToolStat summarizes tool calls whose args are empty or non-object.
type EmptyToolStat struct {
	Agent  string
	Tool   string
	Count  int
	MaxRun int
...
// ContextRewriteStat summarizes slog context rewrite events without carrying
// any prompt, story text, or message content.
type ContextRewriteStat struct {
	Agent            string
	Reason           string
	Strategy         string
	Committed        bool
	Count            int
	LastTokensBefore int
	LastTokensAfter  int
	LastCompacted    int
	LastKept         int
}
...
func captureSessions(dir string, rc *RuntimeCapture) {
	...
	emptyTools := map[string]*EmptyToolStat{}
	dups := map[string]int{}
	...
	for _, f := range files {
		...
		// 不是循环；真死循环是近端高度集中。
		window := tailEvents(evs, repeatWindow)
		aggregateRepeats(f.agent, window, repeats, dups)
		observeEmptyToolCalls(f.agent, window, emptyTools)
	...
	rc.EmptyToolCalls = topEmptyToolCalls(emptyTools)
...
func observeEmptyToolCalls(agent string, evs []SkelEvent, stats map[string]*EmptyToolStat) {
	var current string
	var run int
	for _, ev := range evs {
		for _, t := range ev.Tools {
			key := agent + "\x00" + t.Name
			if len(t.Args) == 0 && !t.Invalid {
				stat := stats[key]
				if stat == nil {
					stat = &EmptyToolStat{Agent: agent, Tool: t.Name}
					stats[key] = stat
				}
				stat.Count++
				if key == current {
...
func captureLog(dir string, rc *RuntimeCapture) {
	...
	for sc.Scan() {
		...
		if rewrite, ok := parseContextRewriteLog(line); ok {
			key := strings.Join([]string{rewrite.Agent, rewrite.Reason, rewrite.Strategy, strconv.FormatBool(rewrite.Committed)}, "\x00")
			stat := contextRewrites[key]
			if stat == nil {
				cp := rewrite
				stat = &cp
				contextRewrites[key] = stat
			}
			stat.Count++
			stat.LastTokensBefore = rewrite.LastTokensBefore
			stat.LastTokensAfter = rewrite.LastTokensAfter
			stat.LastCompacted = rewrite.LastCompacted
			stat.LastKept = rewrite.LastKept
		}
	}
...
func parseContextRewriteLog(line string) (ContextRewriteStat, bool) {
	if !strings.Contains(line, "上下文重写") {
		return ContextRewriteStat{}, false
	}
	fields := parseSlogTextFields(line)
	if fields["module"] != "context" {
		return ContextRewriteStat{}, false
	}
	...
	committed, _ := strconv.ParseBool(fields["committed"])
	return ContextRewriteStat{
		Agent:            fields["agent"],
		Reason:           fields["reason"],
		Strategy:         strategy,
		Committed:        committed,
		LastTokensBefore: atoiDefault(fields["tokens_before"]),
		LastTokensAfter:  atoiDefault(fields["tokens_after"]),
		LastCompacted:    atoiDefault(fields["compacted"]),
		LastKept:         atoiDefault(fields["kept"]),
	}, true
}
...
func sortedContextRewrites(m map[string]*ContextRewriteStat) []ContextRewriteStat {
	if len(m) == 0 {
		return nil
	}
	out := make([]ContextRewriteStat, 0, len(m))
	for _, stat := range m {
		out = append(out, *stat)
	}
...
func topEmptyToolCalls(m map[string]*EmptyToolStat) []EmptyToolStat {
	var out []EmptyToolStat
	for _, stat := range m {
		if stat.Count >= repeatMin || stat.MaxRun >= repeatMin {
			out = append(out, *stat)
		}
	}
	sort.Slice(out, func(i, j int) bool {
		if out[i].MaxRun != out[j].MaxRun {
			return out[i].MaxRun > out[j].MaxRun
		}
		if out[i].Count != out[j].Count {
			return out[i].Count > out[j].Count
		}
