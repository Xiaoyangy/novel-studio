...
func novelContextCall(args string) agentcore.Message {
	return agentcore.Message{
		Role:    agentcore.RoleAssistant,
		Content: []agentcore.ContentBlock{agentcore.ToolCallBlock(agentcore.ToolCall{Name: "novel_context", Args: json.RawMessage(args)})},
	}
}

func toolResult(msg string) agentcore.Message {
	return agentcore.Message{
...
func TestExport_EmptyNovelContextLoop(t *testing.T) {
	var msgs []agentcore.Message
	for range 5 {
		msgs = append(msgs, novelContextCall(`{}`))
		msgs = append(msgs, toolResult("[session_compact: novel_context 128B | ok]"))
	}
	...
	if len(rc.EmptyToolCalls) == 0 {
		t.Fatalf("expected empty tool call stats")
	}
	if !strings.Contains(out, "空参 novel_context 重复读取") {
		t.Fatalf("expected EmptyNovelContextLoop finding\n%s", out)
	}
	if !strings.Contains(out, "`coordinator · novel_context{}` ×5，连续×5") {
		t.Fatalf("expected empty tool aggregate in export\n%s", out)
	}
}
