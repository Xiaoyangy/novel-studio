...
func RenderExport(rep Report, rc RuntimeCapture) []byte {
	...
	if len(rc.EmptyToolCalls) > 0 {
		b.WriteString("- 空参数工具调用（近端窗口 ≥3 次，仅保留工具名）：\n")
		for _, stat := range rc.EmptyToolCalls {
			fmt.Fprintf(&b, "  - `%s · %s{}` ×%d，连续×%d\n", stat.Agent, stat.Tool, stat.Count, stat.MaxRun)
		}
		wrote = true
	}
	...
	if len(rc.ContextRewrites) > 0 {
		...
		for _, stat := range rc.ContextRewrites {
			fmt.Fprintf(&b, "  - %s/%s reason=%s committed=%t ×%d tokens %d→%d compacted=%d kept=%d\n",
				orDash(stat.Agent), orDash(stat.Strategy), orDash(stat.Reason), stat.Committed, stat.Count,
				stat.LastTokensBefore, stat.LastTokensAfter, stat.LastCompacted, stat.LastKept)
		}
		wrote = true
	}
	if rc.StopGuard > 0 {
