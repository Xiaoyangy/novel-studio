...
// projectConfigPath returns the nearest project config from cwd upward. Novel
// runs execute inside data/runs/<book>, while the repository config usually
// lives at the workspace root; checking cwd only silently selected the global
// model roles and could send Writer or Reviewer work to the wrong provider.
// The home-level config is skipped because it was already loaded as global.
func projectConfigPath() string {
	cwd, err := os.Getwd()
	if err != nil || strings.TrimSpace(cwd) == "" {
		return filepath.Join(configDirName, "config.json")
	}
	global, _ := filepath.Abs(DefaultConfigPath())
	for dir := filepath.Clean(cwd); ; dir = filepath.Dir(dir) {
		candidate := filepath.Join(dir, configDirName, "config.json")
	...
	return filepath.Join(cwd, configDirName, "config.json")
}
...
func mergeRAGConfig(base, overlay RAGConfig) RAGConfig {
	if overlay.Embedding != (RAGEmbeddingConfig{}) {
		if overlay.Embedding.Enabled {
			base.Embedding.Enabled = true
		}
		if overlay.Embedding.Provider != "" {
			base.Embedding.Provider = overlay.Embedding.Provider
		}
		...
		if overlay.Embedding.APIKeyEnv != "" {
			base.Embedding.APIKeyEnv = overlay.Embedding.APIKeyEnv
		}
		if overlay.Embedding.BaseURL != "" {
			base.Embedding.BaseURL = overlay.Embedding.BaseURL
		}
		if overlay.Embedding.TimeoutSeconds > 0 {
			base.Embedding.TimeoutSeconds = overlay.Embedding.TimeoutSeconds
		}
		if overlay.Embedding.BuildConcurrency > 0 {
			base.Embedding.BuildConcurrency = overlay.Embedding.BuildConcurrency
		}
		if overlay.Embedding.SearchConcurrency > 0 {
			base.Embedding.SearchConcurrency = overlay.Embedding.SearchConcurrency
		}
	}
	if overlay.Qdrant != (RAGQdrantConfig{}) {
		if overlay.Qdrant.Enabled {
			base.Qdrant.Enabled = true
		}
		if overlay.Qdrant.URL != "" {
			base.Qdrant.URL = overlay.Qdrant.URL
		}
		if overlay.Qdrant.Collection != "" {
			base.Qdrant.Collection = overlay.Qdrant.Collection
		}
		if overlay.Qdrant.APIKey != "" {
			base.Qdrant.APIKey = overlay.Qdrant.APIKey
		}
		if overlay.Qdrant.APIKeyEnv != "" {
			base.Qdrant.APIKeyEnv = overlay.Qdrant.APIKeyEnv
		}
		if overlay.Qdrant.AutoStart {
			base.Qdrant.AutoStart = true
		}
		if overlay.Qdrant.BinaryPath != "" {
			base.Qdrant.BinaryPath = overlay.Qdrant.BinaryPath
		}
		if overlay.Qdrant.DockerImage != "" {
			base.Qdrant.DockerImage = overlay.Qdrant.DockerImage
		}
		if overlay.Qdrant.ContainerName != "" {
			base.Qdrant.ContainerName = overlay.Qdrant.ContainerName
		}
		if overlay.Qdrant.StorageDir != "" {
			base.Qdrant.StorageDir = overlay.Qdrant.StorageDir
		}
		if overlay.Qdrant.TimeoutSeconds > 0 {
