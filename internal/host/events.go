...
func BuildStartPrompt(prompt string) string {
	...
	return "请根据以下创作要求开始创作一部小说。进入规划后，Premise 第一行必须输出 `# 书名`。先判断目标篇幅与结构：设计总字数小于等于 30000 字或明确短篇/单卷时走短篇扁平大纲；设计总字数大于 30000 字、长篇连载、多卷多弧或长期升级关系时走分层长篇结构。章节数量由你根据故事需要和字数预算反推，不能把长篇压缩成短篇式梗概，也不能把短篇拖成长篇开头。\n\n[创作要求]\n" +
		prompt +
