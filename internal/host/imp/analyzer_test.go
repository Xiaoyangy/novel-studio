...
func TestPersistChapter_FullPipeline(t *testing.T) {
	...
	journeys, err := st.LoadSideCharacterJourneys(1)
	if err != nil {
		t.Fatalf("load side character journeys: %v", err)
	}
	if len(journeys) == 0 || journeys[0].Character != "档案馆管理员" {
		t.Fatalf("side character journey not persisted from import: %+v", journeys)
	}
