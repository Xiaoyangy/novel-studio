...
//   - abrupt_strong_event：无铺垫突发强事件，如“隔壁房间忽然砸门”
//   - unsupported_speech_claim：人物声称“我听见你说话了”但需要上文证据
//   - opaque_memo_shorthand：备忘录缩写省掉关键对象或压缩成提纲口吻，如“别回号”“零钱暂不碰”
//   - unit_name_apposition / clipped_habit_sentence：提纲式省略，如“隔壁住户”“搬来两个月，电梯里……”
...
//   - authorial_abstract_summary：动作后用“真正要……从来不是”或“这才让他确定……都没白费”
//     替人物和读者提炼抽象结论
...
//   - dialogue_conveyor_overuse / pov_interiority_thin：群像一人一句交接剧情、
...
var semanticPerplexitySceneMarkers = []string{
	"手机", "电梯", "合同", "收据", "账单", "门牌", "价签", "椅子", "柜台", "纸条", "通知单", "账本", "门缝", "门锁",
	"血印", "雨水", "水龙头", "筷子", "照片", "录音", "小票", "发票", "病历", "货架", "影子", "卡片", "通知单", "门禁卡",
	"工牌", "便利店", "医院", "公寓", "收银台", "纸箱", "“", "”", "「", "」",
}
...
var dialogueMicroPeriodQuoteRe = regexp.MustCompile(`[“「]([^”」\n]+)[”」]`)
var dialogueMicroPeriodExempt = map[string]bool{
...
var abstractSystemReassuranceRe = regexp.MustCompile(`(?:钱没跑|陪你(?:换条路|找(?:条)?路)|规矩不撤|先喘(?:半)?口气|这笔不能这么花|(?:这回)?算你[^。！？!?]{0,8}挣来的)`)
var aphoristicNarrativeSummaryRe = regexp.MustCompile(`(?:理由一条比一条[^。！？!?]{0,24}只有[^。！？!?]{0,24}|[^。！？!?]{0,18}只能[^，。！？!?]{1,16}[，,][^。！？!?]{0,8}不能[^。！？!?]{1,20}|[^。！？!?]{2,12}不等于[^。！？!?]{2,16})`)
var authorialAbstractSummaryRe = regexp.MustCompile(`(?:(?:今天|今晚|眼下|此刻|现在|这一刻|到头来)?(?:他|她|我|[\p{Han}]{2,3})?真正(?:要解|要解决|想要|想做|该做|需要解决|要面对|要证明)的[，,]?(?:从来|根本|压根)?不是[^。！？!?；;\n]{1,48}|眼前(?:真正)?的[，,]?不是(?:麻烦|问题|阻力|困难|失败|结果|答案|终点|任务|选择|考验|机会|代价)[^。！？!?；;\n]{0,48}|(?:这|那)(?:才|总算)让(?:他|她|我|[\p{Han}]{2,3})(?:确定|明白|意识到|知道|相信|认定)[^。！？!?；;\n]{0,40}(?:全都|都)?(?:没有|没)白费)`)
...
var systemProcedureNarrationRe = regexp.MustCompile(`(?:系统(?:判定|显示|提示)[：:]?[^。！？!?\n]{0,48}(?:(?:任务|交付|核验|审核|验收|结算|额度|权限|资格|状态)(?:已)?(?:新增|进入|通过|完成|解锁|开放|生效)|(?:新增|进入|通过|完成|解锁|开放|生效)(?:任务|交付|核验|审核|验收|结算|额度|权限|资格|状态))|(?:任务|项目|执行)?阶段[^。！？!?\n]{0,12}(?:核验|审核|验收)[^。！？!?\n]{0,8}(?:通过|完成)|(?:专项|可用|临时|小额)?(?:额度|权限|资格)[^。！？!?\n]{0,6}(?:解锁|开放|生效))`)
...
func appendSystemMessageSignals(vs []Violation, text string) []Violation {
	...
	for _, match := range systemMessageRe.FindAllStringSubmatch(text, -1) {
		...
		if messageLen < 90 || clauseBreaks < 8 ||
		...
		vs = append(vs, Violation{
			...
			Limit:    "人格对白不要在一条超长消息里同时解释规则、安慰、派任务和发奖励；正式任务卡可完整列必要数字",
			Actual:   clauseBreaks,
			Severity: SeverityWarning,
		})
		break
...
func appendHumanFeelStructureSignals(vs []Violation, text string) []Violation {
	...
	vs = appendRuleCarrierTOSBlock(vs, text)
...
func appendRuleCarrierTOSBlock(vs []Violation, text string) []Violation {
	lines := strings.Split(text, "\n")
	for i := 0; i < len(lines); i++ {
		...
		carrier := containsAnyPhrase(context+joined, []string{"系统", "界面", "页面", "屏幕", "卡面", "通知", "规则", "条款", "任务", "结算"})
		termKinds := 0
		for _, marker := range []string{"条件", "范围", "权限", "额度", "时限", "有效", "交易", "账单", "奖励", "任务", "确认", "责任", "限制", "状态"} {
			if strings.Contains(joined, marker) {
				termKinds++
			}
		}
		if carrier && termKinds >= 2 {
			vs = append(vs, Violation{
				Rule:     "card_tos_block",
				Target:   truncateRunes(joined, 100),
				Limit:    "规则载体或系统提示不能完整 ToS 式列项",
				Actual:   len(block),
				Severity: SeverityWarning,
			})
			return vs
...
func appendAntiAIPhraseSignals(vs []Violation, text string) []Violation {
	...
	// Only inspect narrator text. A character can make a literal correction in
	// dialogue; the prohibited shape is the narrator closing an action with a
	// totalising explanation. Keeping this separate from not_but_overuse also
	// protects concrete scene contrasts such as “最难装的不是牌，是旧桌子”.
	narrative := stripQuotedText(stripMarkdownHeadingLines(text))
	for _, match := range authorialAbstractSummaryRe.FindAllString(narrative, -1) {
		vs = append(vs, Violation{
			Rule:     "authorial_abstract_summary",
			Target:   truncateRunes(strings.TrimSpace(match), 100),
			Limit:    "动作和后果自己成立，不追加替人物或读者提炼意义的抽象结论",
			Actual:   1,
			Severity: SeverityError,
		})
	}
	for _, paragraph := range narrativeParagraphs(text) {
		...
		for _, match := range aphoristicNarrativeSummaryRe.FindAllString(paragraph, -1) {
			vs = append(vs, Violation{
				Rule:     "aphoristic_narrative_summary",
				Target:   truncateRunes(strings.TrimSpace(match), 100),
				Limit:    "人物判断落到当下误判和下一动作，不提炼成可摘抄的对称结论",
				Actual:   1,
				Severity: SeverityWarning,
			})
		}
...
func appendCadenceSignals(vs []Violation, text string) []Violation {
	...
	vs = appendDialogueMicroPeriodChain(vs, text)
	vs = appendDialogueActionLeadRepetition(vs, text)
	vs = appendDialogueConveyorAndInteriority(vs, text)
...
// dialogueMicroPeriodChain finds a deliberately narrow oral-cadence shape:
// inside one character quote, a non-final full stop follows only two to four
// Hanzi, and the same quote then continues. Ordinary short replies, particles,
// questions, exclamations, ellipses and system-message text are deliberately
// outside the signal. Each dialogue paragraph contributes at most one hit, so
// speech tags that split one turn into two quote spans cannot inflate it.
func dialogueMicroPeriodChain(text string) dialogueMicroPeriodStats {
	clean := stripSystemMessageText(text)
	stats := dialogueMicroPeriodStats{}
	for _, paragraph := range narrativeParagraphs(clean) {
		for _, match := range dialogueMicroPeriodQuoteRe.FindAllStringSubmatch(paragraph, -1) {
			if len(match) < 2 {
				continue
			}
			message := strings.TrimSpace(match[1])
			...
			// A speech tag may split one person's turn into two quote spans in
			// the same paragraph. Count that paragraph once.
			break
		}
	}
	return stats
...
func dialogueTurnHasMicroPeriodLead(message string) bool {
	...
	for i, r := range runes {
		switch r {
		case '。':
			// A final short sentence is ordinary dialogue. This signal only
			// covers a tiny period-delimited lead followed by more speech in
			// the same turn.
			if countCJKRunes(runes[i+1:]) == 0 {
				start = i + 1
				continue
			}
			candidate := cjkOnly(runes[start:i])
			if n := len([]rune(candidate)); n >= 2 && n <= 4 && !dialogueMicroPeriodExempt[candidate] {
				return true
			}
			start = i + 1
		case '！', '!', '？', '?', '；', ';', '…':
			// Question, exclamation and unfinished-speech punctuation carry
			// audible prosody and must not be reclassified as period chopping.
			start = i + 1
		}
	}
	return false
...
func cjkOnly(runes []rune) string {
	var b strings.Builder
	for _, r := range runes {
		if r >= '\u4e00' && r <= '\u9fff' {
			b.WriteRune(r)
...
func countCJKRunes(runes []rune) int {
	count := 0
	for _, r := range runes {
		if r >= '\u4e00' && r <= '\u9fff' {
			count++
...
func appendDialogueMicroPeriodChain(vs []Violation, text string) []Violation {
	stats := dialogueMicroPeriodChain(text)
	if stats.HitTurns < 3 {
		return vs
	}
	return append(vs, Violation{
		Rule:     "dialogue_micro_period_chain",
		Target:   strings.Join(stats.Examples, " / "),
		Limit:    "同章不同话轮中二至四字句号短句 < 3；短答、语气词和问叹/未尽语气不计",
		Actual:   stats.HitTurns,
		Severity: SeverityWarning,
	})
}
...
func appendDialogueConveyorAndInteriority(vs []Violation, text string) []Violation {
	paragraphs := narrativeParagraphs(text)
	if len(paragraphs) < 8 {
		return vs
	}
	...
	for _, paragraph := range paragraphs {
		...
		if len(examples) < 4 {
			examples = append(examples, truncateRunes(trimmed, 44))
		}
	}

	denseWindows := 0
	for start := 0; start+8 <= len(paragraphs); start++ {
		window := paragraphs[start : start+8]
		windowDialogue := 0
		for _, paragraph := range window {
			if strings.ContainsAny(paragraph, "“「") {
				windowDialogue++
			}
		}
	...
	if (denseWindows > 0 && compoundConveyor) || (dialogueCount >= 10 && maxRun >= 5 && compoundConveyor) {
		vs = append(vs, Violation{
			...
			Target:   strings.Join(examples, " / "),
			Limit:    "任意 8 段内对白推进段 < 6，或必须有主视角体验/沉默/后果换挡",
			Actual:   max(denseWindows, 1),
			Severity: SeverityWarning,
		})
	}
	...
	if hanziCount >= 1800 && dialogueCount >= 8 && logisticsCount >= 8 && interiorityCount < 2 {
		vs = append(vs, Violation{
			...
			Limit:    "至少让两处主观体验改变注意力、选择或关系判断；不能只补情绪标签和微动作",
			Actual:   interiorityCount,
			Severity: SeverityWarning,
		})
	}
	return vs
...
func appendIsolatedSentenceOveruse(vs []Violation, text string) []Violation {
	...
	for i, paragraph := range paragraphs {
		...
		// 移动端网文里“一句一段”本身是正常排版。这里只拦连续的极短碎片，
		// 例如“他停住手。/门关上。/三户。/不是一户。”式分镜提纲。
		if sentenceCount == 1 && countCJK(paragraph) <= 12 {
			count++
			run++
			if run > maxRun {
				maxRun = run
	...
	limit := len(paragraphs) / 5
	if limit < 6 {
		limit = 6
	}
	if count <= limit && maxRun < 4 {
		return vs
	}
	return append(vs, Violation{
		Rule:     "isolated_sentence_overuse",
		Target:   fmt.Sprintf("%s（最长连续 %d 段）", strings.Join(examples, " / "), maxRun),
		Limit:    fmt.Sprintf("极短单句段 <= %d 且连续 < 4", limit),
		Actual:   count,
		Severity: SeverityWarning,
	})
}
...
func appendObjectResponseIssues(vs []Violation, text string) []Violation {
	rawMatches := objectResponseRe.FindAllString(text, -1)
	matches := make([]string, 0, len(rawMatches))
	for _, match := range rawMatches {
		// "手机没有动静" is an absence beat, not a fifth object
		// response. The broad response regex otherwise counts the "动" in
		// "动静", making the required absence proof inflate the overuse
		// counter it is meant to fix.
		if objectResponseAbsenceRe.FindString(match) != "" {
			continue
		}
