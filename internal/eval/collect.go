...
func collectRAG(s *store.Store, check func(string, error)) RAGCollection {
	var out RAGCollection
	state, err := s.RAG.LoadIndexState()
	check("rag_index_state", err)
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		out.IndexLoadError = err.Error()
	}
	out.IndexState = state

	vectorStore, err := s.RAG.LoadVectorStore()
	check("rag_vector_store", err)
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		out.VectorStoreLoadErr = err.Error()
	}
	out.VectorStore = vectorStore
	...
	if state != nil {
		if cfg.QdrantURL == "" {
			cfg.QdrantURL = state.Config.QdrantURL
		}
		if cfg.Collection == "" {
			cfg.Collection = state.Config.Collection
		}
		if cfg.VectorStore == "" {
			cfg.VectorStore = state.Config.VectorStore
		}
	}
	if cfg.VectorStore != "qdrant" || cfg.QdrantURL == "" || cfg.Collection == "" {
		return out
	}
	...
	out.QdrantHealthy = true
	count, err := client.Count(ctx, true)
	if err != nil {
		out.QdrantError = err.Error()
		return out
	}
	out.QdrantPoints = count
	return out
}
