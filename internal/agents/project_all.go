...
func RunProjectedChapterPlanning(
	...
	if plan == nil {
		...
		resumeRaw, resumeAttempted, resumeErr := tools.NewPlanDetailsTool(st).
			WithGroundingReviewer(NewPlanGroundingReviewer(cfg, models, nil)).
			ResumePartial(ctx, chapter, planningContextRaw)
		if resumeErr != nil && !tools.PlanDetailsResumeNeedsRepair(resumeErr) {
			return nil, resumeErr
		}
		if resumeAttempted && resumeErr == nil {
			var resumed struct {
				Planned bool `json:"planned"`
			}
			if err := json.Unmarshal(resumeRaw, &resumed); err != nil || !resumed.Planned {
				return nil, fmt.Errorf("project-all partial resume chapter %d did not finalize: %s", chapter, resumeRaw)
			}
			...
			if plan == nil || planCP == nil {
				return nil, fmt.Errorf("project-all partial resume chapter %d lacks a current plan/checkpoint", chapter)
			}
		}
		if plan == nil {
			if err := projectedAccountingBefore(ctx); err != nil {
			...
			thinking, _ := ResolveThinkingForModel(model, roleThinking(cfg, "writer"))
			plannerPrompt := projectAllPlannerPrompt(chapter, planningContextRaw, arcBoundary, bookBudget, resumeErr)
