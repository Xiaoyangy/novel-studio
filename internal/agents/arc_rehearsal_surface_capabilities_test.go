...
func TestSurfaceRehearsalProfilePreservesEveryHistoricalProducer(t *testing.T) {
	...
	for _, producer := range CharacterActivationProducerCandidates(domain.CharacterActivationCyclePolicyV3) {
		...
		selectionMust(t, err)
		if producer == characterActivationProtocolV3Digest() || producer == characterActivationProtocolV3IncomingReadDigest() || producer == characterActivationProtocolV3InitialSelfIntentDigest() || producer == characterActivationProtocolV3MemoryTextDigest() {
