...
func TestContinuationProducerRuntimeRecoversOldAndNewFrozenWork(t *testing.T) {
	for _, full := range []bool{false, true} {
		t.Run(map[bool]string{false: "legacy", true: "full-owner"}[full], func(t *testing.T) {
			...
			for _, cycle := range proof.Cycles {
				if cycle.Evidence.ProtocolDigest != producer || domain.HasCharacterWorkContinuationHistoryPolicyV1(cycle.Evidence.Stimulus.Sources) != full {
