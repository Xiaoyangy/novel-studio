...
type writerStoreSummaryState struct {
	...
	writingEngine     *writerWritingEngine
	styleRules        *domain.WritingStyleRules
	pendingReviews    []writerPendingReview
}

type writerWritingEngine struct {
	EnabledFeatures []writerWritingFeatureRef `json:"enabled_features,omitempty"`
	...
	Feedback        []writerWritingFeedback   `json:"feedback,omitempty"`
	Trace           []string                  `json:"trace,omitempty"`
}

type writerWritingFeatureRef struct {
	ID       string `json:"id"`
	Name     string `json:"name,omitempty"`
	Category string `json:"category,omitempty"`
}

type writerWritingFeedback struct {
...
func loadWriterStoreSummaryState(s *store.Store) (*writerStoreSummaryState, bool, error) {
	...
	state.writingEngine, err = loadWriterWritingEngine(s, progress, chapter)
	if err != nil {
		return nil, false, err
	}
...
func loadWriterRestoreState(s *store.Store) (*writerStoreSummaryState, error) {
	...
	state.writingEngine, _ = loadWriterWritingEngine(s, progress, chapter)
...
func writerStoreSummarySections(state *writerStoreSummaryState) []writerStoreSection {
	return []writerStoreSection{
		...
		{heading: "写法引擎", data: state.writingEngine},
...
func loadWriterWritingEngine(s *store.Store, progress *domain.Progress, chapter int) (*writerWritingEngine, error) {
	if s == nil || progress == nil || chapter <= 0 {
		return nil, nil
	}
	scope := &domain.WritingBinding{
		Scope:   "chapter",
		Volume:  progress.CurrentVolume,
		Arc:     progress.CurrentArc,
		Chapter: chapter,
	}
	compiled, err := s.WritingAssets.CompileForScope(1, scope)
	if err != nil || compiled == nil {
		return nil, err
	}
	return compactWriterWritingEngine(compiled), nil
}

func compactWriterWritingEngine(compiled *domain.WritingCompiled) *writerWritingEngine {
	if compiled == nil {
		return nil
	}
	if len(compiled.EnabledFeatures) == 0 &&
		len(compiled.ActiveRules) == 0 &&
		len(compiled.AntiAIRules) == 0 &&
		len(compiled.Taboos) == 0 &&
		len(compiled.Feedback) == 0 {
		return nil
	}
	engine := &writerWritingEngine{
		ActiveRules: appendLimitedStrings(compiled.ActiveRules, 16),
		AntiAIRules: appendLimitedStrings(compiled.AntiAIRules, 8),
		Taboos:      appendLimitedStrings(compiled.Taboos, 8),
		Trace:       appendLimitedStrings(compiled.Trace, 8),
	}
	limit := min(len(compiled.EnabledFeatures), 12)
	engine.EnabledFeatures = make([]writerWritingFeatureRef, 0, limit)
	for _, feature := range compiled.EnabledFeatures[:limit] {
		engine.EnabledFeatures = append(engine.EnabledFeatures, writerWritingFeatureRef{
			ID:       feature.ID,
			Name:     feature.Name,
			Category: feature.Category,
		})
	}
	feedbackLimit := min(len(compiled.Feedback), 6)
	engine.Feedback = make([]writerWritingFeedback, 0, feedbackLimit)
	for _, item := range compiled.Feedback[:feedbackLimit] {
		engine.Feedback = append(engine.Feedback, writerWritingFeedback{
...
func appendLimitedStrings(values []string, limit int) []string {
	if len(values) == 0 || limit <= 0 {
		return nil
	}
	return append([]string(nil), values[:min(len(values), limit)]...)
}
