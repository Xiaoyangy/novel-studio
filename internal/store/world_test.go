...
func TestReview_ArcScopeIsolation(t *testing.T) {
	...
	_ = s.World.SaveReview(domain.ReviewEntry{Chapter: 8, Scope: "arc", Verdict: "accept"})

	loaded, _ := s.World.LoadReview(8)
	if loaded == nil || loaded.Scope != "chapter" || loaded.Verdict != "accept" {
		t.Fatalf("chapter review should remain isolated from arc review, got %+v", loaded)
	}
	if !s.World.HasArcReview(8) {
		t.Fatal("arc review should be found from isolated arc review path")
	}
}
