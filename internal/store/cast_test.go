...
func TestCastSaveNilWritesEmptyArray(t *testing.T) {
	s := NewStore(t.TempDir())
	if err := s.Cast.Save(nil); err != nil {
		t.Fatalf("save nil: %v", err)
	}
	data, err := os.ReadFile(filepath.Join(s.Dir(), "meta", "cast_ledger.json"))
	if err != nil {
		t.Fatalf("read: %v", err)
	}
	if strings.TrimSpace(string(data)) == "null" {
		t.Fatalf("nil 名册不应序列化为字面量 null: %s", data)
	}
	entries, err := s.Cast.Load()
