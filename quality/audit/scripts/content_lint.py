...
def cadence_issues(raw: str) -> list[dict]:
    ...
    for para in paras:
        if any(mark in para for mark in "“”「」【】"):
            continue
        for match in APHORISTIC_NARRATIVE_SUMMARY_RE.finditer(para):
            issues.append(
                {
                    "rule": "aphoristic_narrative_summary",
                    "severity": "warning",
                    "line": line_number(raw, raw.find(para) + match.start()),
                    "limit": "人物判断落到当下误判和下一动作，不提炼成可摘抄的对称结论",
                    "actual": 1,
                    "target": match.group(0)[:100],
                    "evidence": "X只能Y不能Z、理由很正经只有某项漏算等句式会把实时心理磨成作者总结",
                }
            )
    ...
    tiny_isolated: list[str] = []
    tiny_run = 0
    max_tiny_run = 0
    for index, para in enumerate(paras):
        stripped = para.strip()
        is_title = index == 0 and re.match(r"^#*\s*第?[零一二三四五六七八九十百千万0-9]+章(?:\s|$)", stripped)
        is_dialogue = bool(re.fullmatch(r"[“「][\s\S]*[”」][。！？!?]?", stripped))
        is_system = bool(re.fullmatch(r"【[^】]+】", stripped))
        cjk_count = len(re.findall(r"[\u4e00-\u9fff]", stripped))
        if not (is_title or is_dialogue or is_system) and len(split_sentences(para)) == 1 and cjk_count <= 12:
            tiny_isolated.append(para)
            tiny_run += 1
            max_tiny_run = max(max_tiny_run, tiny_run)
        else:
            tiny_run = 0
    tiny_limit = max(6, len(paras) // 5)
    if len(tiny_isolated) > tiny_limit or max_tiny_run >= 4:
        issues.append(
            {
                "rule": "isolated_sentence_overuse",
                "severity": "warning",
                "line": line_number(raw, raw.find(tiny_isolated[0])),
                "limit": f"极短单句段<={tiny_limit}且连续<4",
                "actual": len(tiny_isolated),
                "target": " / ".join(para[:40] for para in tiny_isolated[:3]),
                "evidence": f"移动端网文允许自然的一句一段；这里只拦12字内极短碎句成串（最长连续{max_tiny_run}段）",
            }
        )
