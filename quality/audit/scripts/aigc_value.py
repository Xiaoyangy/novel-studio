...
INTERIORITY_HINTS = [
    "心里", "心口", "胸口", "脑子里", "以为", "觉得", "他想", "她想", "本想",
    "没想到", "想起", "想到", "记得", "第一反应", "第二反应", "拿不准", "不准备",
    "不打算", "打定主意", "宁愿", "巴不得", "恨不得", "难堪", "委屈", "后悔",
    "犹豫", "不甘", "不愿", "不敢", "舍不得", "偏偏", "明明", "早知道", "差点",
    "松了口气", "说不上来",
]
...
def dialogue_micro_period_chain_stats(text: str) -> dict:
    """Mirror Go's narrow 2-4-Hanzi non-final-period dialogue signal."""
    ...
    for paragraph in paragraphs(clean):
        for _, _, message in dialogue_quote_spans(paragraph):
            ...
            if has_hit:
                # Speech tags can split one turn into several quote spans in
                # one paragraph; match Go by counting the paragraph once.
...
def score_narrative_dynamics(
    body: str,
    n_hanzi: int,
    micro_period_stats: dict | None = None,
) -> dict:
    paras = paragraphs(body)
    micro_period_stats = micro_period_stats or dialogue_micro_period_chain_stats(body)
    dialogue_paras = 0
    action_lead_paras = 0
    max_dialogue_run = 0
    current_run = 0
    ...
    signals = []
    if micro_period_stats.get("turn_count", 0) >= 3:
        signals.append(
            risk_item(
                "dialogue_micro_period_chain",
                0.64,
                f"{micro_period_stats.get('turn_count', 0)} 个不同对白话轮在同一引号内"
                "用二至四字句号短句切开后续表达",
            )
        )
    ...
    return {
        ...
        "stats": {
            ...
            "dialogue_micro_period_chain_turns": micro_period_stats.get("turn_count", 0),
            "dialogue_micro_period_chain_examples": micro_period_stats.get("examples", []),
...
def whole_text_raw_curve_consensus_floor(
    probability_curve: float, weak_lm: float, entropy: float
) -> tuple[float, bool]:
    """Return the candidate floor when all three raw curve views agree.

    The three views share low-level surprisal/entropy inputs, so consensus is
    necessary but not sufficient for a hard gate.  The caller must also require
    an independent narrative or structural signal.
    """
    if probability_curve < 90 or weak_lm < 90 or entropy < 90:
        return 0.0, False
    consensus_mean = (probability_curve + weak_lm + entropy) / 3
    floor = min(86.0, max(76.0, 60.0 + consensus_mean * 0.20))
    return round(floor, 2), True


def whole_text_independent_risk_support(
    narrative_dynamics: float,
    structure: float,
    burstiness: float,
    cross_paragraph: float,
) -> bool:
    """Return independent support required before raw curves become a floor."""
    return (
        # A lone borderline POV/interiority heuristic scores 46. Keep it as
        # rewrite advice, but do not let it turn three correlated curve views
        # into a 76%+ hard floor. Compound narrative risks score at least 55.
        narrative_dynamics >= 55
        or structure >= 65
        or (
            burstiness >= 35
            and (structure >= 45 or cross_paragraph >= 45)
        )
    )


def segment_aigc_proxy(
    segment_report: dict, char_count: int, proportion: float
    ...
    if 1800 <= char_count <= 3600 and proportion >= 0.95 and narrative_like and anchor_type != "technical_expository":
        ...
        consensus_floor, raw_curve_consensus = whole_text_raw_curve_consensus_floor(
            raw_probability_curve, raw_weak, raw_entropy
        )
        independent_hard_support = whole_text_independent_risk_support(
            narrative_dynamics, structure, burstiness, cross_paragraph
        )
        independent_support = sum(
            (
                current_curve >= 60,
                zhuque_composite >= 50,
                burstiness >= 35 and (structure >= 45 or cross_paragraph >= 45),
        ...
        if raw_curve_consensus and independent_hard_support:
            whole_text_hard_gate = True
            score = max(score, consensus_floor)
            evidence.append(
                "整章单段三条原始曲线同时高危，且存在独立叙事或结构风险，形成复合风险下限"
            )
        elif raw_curve_consensus:
            evidence.append(
                "整章单段三条原始曲线同时偏高，但缺少独立叙事或结构风险，不形成硬门禁"
            )
        elif raw_curve >= 80 and multi_signal_support:
            ...
            evidence.append(
                f"整章单段仅有部分原始曲线高值 {raw_curve:.2f}%，未形成三曲线复合风险"
            )
    blockers = human_anchor.get("blockers", [])
    ...
    if segment_anchor_eligible and not whole_text_hard_gate:
    ...
    return round(clamp(score / 100) * 100, 2), evidence, whole_text_hard_gate


def zhuque_segment_risk_floor(
    segments: list[dict], max_score: float, risk_ratio: float
) -> float:
    """Return only verified hard floors; keep other segment scores diagnostic."""
    if len(segments) == 1:
        segment = segments[0]
        if (
            segment.get("whole_text_hard_gate")
            and risk_ratio >= 99
            and max_score >= 50
        ):
            return max_score
        return 0.0
    if max_score >= 80 and risk_ratio >= 60:
        return round(risk_ratio * 0.90, 2)
    if max_score >= 60 and risk_ratio >= 35:
        return round(risk_ratio * 0.70, 2)
    return 0.0
...
def zhuque_segment_proxy(raw: str) -> dict:
    ...
    for index, (start, end) in enumerate(bounds, 1):
        ...
        score, evidence, whole_text_hard_gate = segment_aigc_proxy(
            report, segment_chars, proportion
        )
        if score >= 99:
        ...
        segments.append(
            {
                ...
                "evidence": evidence,
                "whole_text_hard_gate": whole_text_hard_gate,
    ...
    risk_floor = zhuque_segment_risk_floor(segments, max_score, risk_ratio)
    return {
        "enabled": True,
        "segments": segments,
...
def whole_text_single_segment_risk(segment_proxy: dict | None) -> float:
    """Return the verified whole-text floor, never an uncorroborated max."""
    ...
    if proportion < 0.95 or not segment.get("whole_text_hard_gate"):
        return 0.0
...
def latest_detector_proxy(
    ...
    components = {
        ...
        "narrative_dynamics": score_narrative_dynamics(body, len(chars), micro_period_stats),
...
def analyze_text(text: str, include_segments: bool = True) -> dict:
    ...
    noise_stats = detector_noise_stats(body)
    micro_period_stats = dialogue_micro_period_chain_stats(body)
    ...
    latest_proxy = latest_detector_proxy(
        body,
        chars,
        sents,
        sent_lens,
    ...
    return {
        ...
        "stats": {
            ...
            "detector_noise": noise_stats,
            "dialogue_ratio": dialogue_ratio,
            "dialogue_micro_period_chain": micro_period_stats,
            "cliche_total_per_k": total_cliche,
            "cliche_per_k": per_k,
            "concrete_density_per_k": concrete_density,
            "action_density_per_k": action_density,
