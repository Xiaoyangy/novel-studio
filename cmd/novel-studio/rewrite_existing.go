...
11. **高质锚点可留其意不留其形**：原文里真正有信息/关系/规则功能的段落（关键对白、证据链、人物选择、章末压力段），保留它承载的事实与功能，但用更自然的文字重新写出来；空对仗、拼接残留、视角硬伤必须改掉。
...
15. **纸面材料要像真实载体**：审计记录、邮件、系统提示、备忘录不要写成"页面写着：A；B；C；D"。优先用标题、栏位、遮挡、回执、涂改、补字、角色逐行读到或被打断的方式呈现。
16. **结构感必须拆散**：遇到并列清单、连续顺序词、工整解释链、连续"X得发Y"、相邻重复对话点、视线/身体方位冲突时必须局部实改。不要为了"人味"随机加错别字；要用划掉、补字、读不全、卡壳、沉默、关系压力和视角合理化来修。
17. **作者画像与专业信息**：默认叙述者背后是 30 岁左右、有文学素养的程序员。她懂算法、字段、审计、权限和系统边界，但正文不能像技术文档；专业名词不必逐个解释，必须通过界面痕迹、字段错位、权限卡点、同事误判、生活动作和后果让非专业读者跟上。
...
3. 红旗修复以替换原有段落完成：首章爽点/章末钩子优先替换信息倾倒段、权限压力段和结尾段，不额外加一整个新场景。
