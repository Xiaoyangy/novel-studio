...
func rewriteExistingPipeline(opts cliOptions, args []string) error {
	...
	if err := ensurePipelineRAGReady(cfg); err != nil {
		return fmt.Errorf("rewrite-existing 写前 RAG 检查失败: %w", err)
	}
	...
	for round := 1; round <= maxRounds; round++ {
		...
		for chNum := start; chNum <= end; chNum++ {
			...
			for attempt := 1; attempt <= 3; attempt++ {
				...
				if err := validatePatchRewriteWithBounds(string(original), newText, bounds); err != nil {
					if isPatchRewriteLengthError(err) && attempt < 3 {
		...
		blocking, err := blockingRevisionChapters(eng.Dir(), start, end)
		if err != nil {
			return err
		}
		if len(blocking) == 0 {
			if err := completeResolvedRewrites(eng.Dir(), start, end); err != nil {
				return err
			}
			fmt.Fprintf(os.Stderr, "[rewrite-existing] 复审无红旗，重写闭环完成\n")
			return nil
		}
		fmt.Fprintf(os.Stderr, "[rewrite-existing] 复审仍有红旗章节：%s\n", formatChapterList(blocking))
		if round == maxRounds {
			return fmt.Errorf("达到最大闭环轮数 %d 后仍有红旗章节：%s", maxRounds, formatChapterList(blocking))
		}
	}
	if totalRewritten == 0 {
...
func completeResolvedRewrites(projectDir string, start, end int) error {
...
const rewriteSystemPrompt = `你是中文长篇小说资深编辑兼主笔。你的任务不是重写一章新稿，而是在用户提供的"原章节正文"上做局部修订：保留原文骨架、顺序、声口、强段落和已发布读感，只修评审指出的必要问题。

## 硬性约束
1. **只服从当前项目**：人物名、组织名、事件、时间点、权限边界、证据状态和章节标题都以用户给出的当前项目材料、原文、本章大纲和评审反馈为准。不得继承其他书、旧归档、示例项目或过期 prompt 的专属设定。
2. **保持原文叙事身份**：原文是第几人称、什么视角、谁的主视角，就保持该视角。不要把第三人称改成第一人称，也不要换主角。
3. **不乱加设定**：可以为了修复评审问题补匿名窗口人员、现场阻力、动作和物件摩擦；不能新增会改变主线事实的人名、组织、合同、授权、证据或能力。
4. **保留核心骨架**：本章关键事件、因果链、伏笔、章末钩子和已入账资源必须保留。改写是压缩重复、增强冲突、替换套话、修正语气和补足评审缺口，不是推倒重来。
5. **证据和权限边界优先**：待核验就是待核验，冻结不等于读取，索引不等于正文，线索不等于责任坐实。不得让角色越权取得原文没有取得的材料。
6. **字数以当前项目为准**：保持原文字数量级，按评审要求适度增删；不要套用其他项目的固定字数模板。
7. **格式保持章节正文**：开头保留或生成本章 markdown 标题，正文直接展开。短段落可读，一段通常 1-3 句。
8. **内容质量第一**：审核通过性不能压过小说质量。AI率目标放宽为不高于 5%，不要追求 <1%。禁止为了检测引入注水、乱码、OCR 脏码、随机汉字、冷僻词堆砌、无信息清单、拟声长串或刻意错别字。
9. **红旗必须修，黄旗择优修**：rewrite brief 里的红旗是阻断项，必须用更好的剧情动作、对话摩擦、证据链或人物选择来修。黄旗只有在能提升人物、节奏、信息清晰度或语言质感时才采纳；如果只是为了指标换词或加噪，宁可保留原文。
10. **最小修订优先**：优先替换少量句子、压缩少量段落、补一两句必要对话或动作。严禁整章重起、换叙事节拍、把原文强段落改成说明文。
11. **保留原文高质锚点**：用户原文中的童谣规则链、自然对白、强物件段和章末压力段必须尽量原样保留；但锚点里的空对仗、拼接残留或视角硬伤若被 rewrite brief / 机械门禁点名，必须局部修。
12. **术语不外溢**：不得把审核建议里的概念词直接写进正文。普通叙述和普通人对话少用"标的"；不得新增"可识别标的""可追索标的""可识别、可追索的标的"这类审计腔表达。需要表达时改成角色能说的话，如"它认的是能追到人的东西"。
...
15. **纸面条款要像真实载体**：账单、黑卡提示、白纸条款、备忘录不要写成"纸面写着：A；B；C；D"。优先用标题+换行栏位、缺字、盖印、涂改、渗水、补字、角色逐行读到或被打断的方式呈现。
16. **结构感必须拆散**：遇到便签三条、黑卡 ToS、工整童谣、连续"X得发Y"、相邻重复骂点、猫眼读背面字、影子/身体方位冲突时必须局部实改。不要为了"人味"随机加错别字；要用划掉、补字、读不全、卡壳、沉默、拖鞋回收、视角合理化来修。
...
13. 分号处理：保留童谣、咒词、角色故意念条款里的必要分号；普通叙述和对白里的分号要优先改成句号、逗号或换行。若同一句出现三个以上分号，除非是受保护童谣，否则必须拆开。
14. 条款/单据处理：诡异文本先给读者看见纸面排版，再让角色反应。栏位信息分行呈现，不要一次性报幕；可以让纸面漏一项、渗出一项、被角色划掉一项，使规则从场景里长出来。
15. 人味对白处理：把"漂亮但不像人说"的句子改成角色会说的话。朋友之间可以抬杠、算账、嘴硬、半开玩笑，但不能整齐到像标语。
16. **黄旗打磨净增限制**：允许为了修人物、冲突和证据链增补文字，但整章最终字数不得低于2300，且相对原文净增不得超过15%。删除不设比例限制。补配角对话、现场误判或交易代价时，同步删掉原有解释、重复异象、无功能短句或抽象比喻，避免只加不减。
...
3. 红旗修复以替换原有段落完成：首章爽点/章末钩子优先替换黑卡段、欠费单段和结尾段，不额外加一整个新场景。
4. 删除重复门响、灯光、门牌、纸页、手机响应；合并连续短句；把"没有/没/停在半空/迟疑"类动作压到一两处。
...
请对 ch%02d 做压缩型返工：先把正文压进字数窗口，再用替换黑卡段/欠费单段/结尾段的方式完成必要红旗修复。不要新增完整场景，不要扩写新人物对白，不要把评审术语写进正文；如果只是黄旗打磨，最终不少于2300字，净增不得超过15%%。`,
			...
			preserveCtx,
			chNum,
			truncateForContext(original, 14000),
			reviewCtx,
			chNum,
		)
...
请按 rewrite brief 对 ch%02d 做局部修订。以原文为底稿，不要直接重生成新章；保留高质锚点和章节骨架。只改确实会影响红旗/质量的句段。若本轮主要是黄旗打磨，最终不少于2300字，净增不得超过15%%：补一处配角冲突或代价提示，就删掉一部分原文解释、重复异象、同义短句或抽象比喻。不要把评审建议里的"可识别/可追索/标的"等概念词直接写进正文。`,
...
func rewritePatchBoundsFor(projectDir, original string) rewritePatchBounds {
	...
	if projectDir != "" && readJSONIfExists(filepath.Join(projectDir, "meta", "user_rules.json"), &snap) && snap.Structured.ChapterWords != nil {
		...
		if wr.Min > 0 && wr.Max > 0 && wr.Min <= wr.Max {
			...
			bounds.Source = fmt.Sprintf("rewrite 改写窗口：最终不少于%d字，净增不超过%d%%；user_rules.chapter_words=%d-%d 仅作参考", rewriteMinFinalWords, rewriteMaxNetGrowthPercent, wr.Min, wr.Max)
...
func rewriteLengthInstruction(bounds rewritePatchBounds, retryFeedback string) string {
	...
	if bounds.ProjectMin > 0 || bounds.ProjectMax > 0 {
		fmt.Fprintf(&b, "- 本书章节字数规则：%d-%d 字；本轮 rewrite 以 2300 保底和 15%% 净增上限为写回校验口径。\n", bounds.ProjectMin, bounds.ProjectMax)
	}
	...
	if bounds.Original > bounds.Max {
		...
		b.WriteString("- 修复红旗时以替换现有段落为主，不新增完整场景；首章爽点/规则钩子优先替换黑卡段和章末段完成。\n")
		b.WriteString("- 优先删除：重复门响/灯光/门牌响应、重启验证、便签复盘、同义迟疑动作、无功能比喻、可合并的单句短段。\n")
	}
	if feedback := strings.TrimSpace(retryFeedback); feedback != "" {
		fmt.Fprintf(&b, "- 上一版写回失败：%s\n", feedback)
		b.WriteString("- 这次优先压缩：保留红旗修复和章末钩子，删解释性复盘、重复恐怖比喻、同义短句和无信息动作。\n")
	}
...
func rewriteLengthRetryFeedback(err error, bounds rewritePatchBounds) string {
	targetMax := rewriteRetryTargetMax(bounds)
	return fmt.Sprintf("%v；请把正文压回 %d-%d 字，内部目标不超过 %d 字，最终绝对不能超过 %d 字。只保留必要红旗修复。", err, bounds.Min, bounds.Max, targetMax, bounds.Max)
}

func rewriteModelRetryFeedback(err error, bounds rewritePatchBounds) string {
	targetMax := rewriteRetryTargetMax(bounds)
	return fmt.Sprintf("上一轮模型调用失败：%v。请重新输出完整章节，遵守 %d-%d 字窗口，内部目标不超过 %d 字。", err, bounds.Min, bounds.Max, targetMax)
}
...
func validatePatchRewriteWithBounds(original, rewritten string, bounds rewritePatchBounds) error {
	...
	for _, anchor := range preserveAnchors(original) {
		if !strings.Contains(rewritten, anchor) {
			return fmt.Errorf("新稿丢失原文高质锚点：%s", truncateForContext(anchor, 120))
		}
	}
