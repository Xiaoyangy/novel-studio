...
7. **格式=正常小说，不是 Markdown**：首行是纯文本章节标题（如「第一章 欠费单」，不要写 # 或任何 Markdown 符号）；段落之间空一行；全程禁止 # * - > 反引号 等标记。
...
11. **高质锚点可留其意不留其形**：原文里真正有信息/关系/规则功能的段落（童谣规则链、关键对白、强物件段、章末压力段），保留它承载的事实与功能，但用更自然的文字重新写出来；空对仗、拼接残留、视角硬伤必须改掉。
