...
20. **正式任务卡与系统人格对白分层**：正式任务卡/结算卡可以紧凑列出当前项目已经确定的目标、时限、奖励，并保留既定数字；不得为了“人味”隐藏数字、拆散任务或让主角自行推断。任务卡之外的系统人格对白每次只回答眼前一个问题，或只给一条规则/一个可执行提示；禁止客服腔和“系统判定/阶段核验通过/本地新增交付/进入核验”这类后台流程腔，可以偏心、吐槽，但不能替主角办事。
...
8. 区分正式任务卡与系统人格对白：任务卡/结算卡允许紧凑列出已经确定的目标、时限、奖励并保留既定数字，不得隐藏数字或让主角自行推断；人格对白每次只回答眼前一个问题，禁客服腔和“系统判定/阶段核验通过/本地新增交付/进入核验”之类后台流程腔。
