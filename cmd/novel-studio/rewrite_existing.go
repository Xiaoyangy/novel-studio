...
func rewriteExistingPipeline(opts cliOptions, args []string) error {
	...
	for round := 1; round <= maxRounds; round++ {
		...
		for _, chNum := range selectedChapters {
			...
			outlineEntry := resolveRewriteOutlineEntry(st, volumes, chNum)
...
// resolveRewriteOutlineEntry falls back to the approved chapter plan when the
// layered outline has not materialized chapter entries. Rewrite must still keep
// the chapter title and core contract, and callers must never dereference nil.
func resolveRewriteOutlineEntry(st *store.Store, volumes []domain.VolumeOutline, chapter int) *domain.OutlineEntry {
	if entry := lookupOutlineEntry(volumes, chapter); entry != nil {
		return entry
	}
	if st == nil {
		return nil
	...
	return &domain.OutlineEntry{
		...
		CoreEvent: strings.TrimSpace(plan.Goal),
		Hook:      strings.TrimSpace(plan.Hook),
		Scenes:    append([]string(nil), plan.Contract.SceneAnchors...),
	}
}
...
8. **内容质量第一**：审核通过性不能压过小说质量。本地与外部 AI率都必须严格小于 4%，4% 也失败。禁止为了检测引入注水、乱码、OCR 脏码、随机汉字、冷僻词堆砌、无信息清单、拟声长串或刻意错别字。
9. **反 AI 味硬要求（这是本次返工的核心）**：句长、段落功能和局部语义要自然换挡；单句成段全章≤4 且绝不连续；先决定谁必须说、谁可以沉默、哪条信息本章不说，禁止把一份说明拆成多人各说一句；一组密集对白只完成一个局面变化，随后回到主角判断、后果行动或现场余波。至少两次让主角的主观体验改变注意、误判、选择或关系理解，不能靠“手指一顿”等微动作补票。
...
17. **作者画像与专业信息**：只采用当前项目明确给出的角色职业、叙述视角和作者声口；不得套用程序员、女性职场或其他项目的默认画像。专业名词不必逐个解释，必须通过界面痕迹、权限卡点、人物误判、生活动作和后果让非专业读者跟上。
18. **对白标签降密**：连续双人对白能靠声口、上一句问题、动作位置和关系压力辨认时，省略部分"他说/她问/他答"；三人以上只在必要处补标签。禁止"人物：台词"剧本格式。动作 beat 只保留会改变局面、遮掩信息、暴露情绪、打断台词或触发规则的动作。
19. **热梗句法不得变形**：若使用“呱，……”这一候选，只能让角色直接开口，逗号后接一条完整吐槽，并让同席人物当场给出一个自然的席间反应。前面不得写“怪叫一声”“叫了一声”“发出一声”或任何拟声动作；一条吐槽加一个反应就够，不要求第二句续梗，生硬就整句删除。
20. **正式任务卡与系统人格对白分层**：正式任务卡/结算卡可以紧凑列出当前项目已经确定的目标、时限、奖励，并保留既定数字；不得为了“人味”隐藏数字、拆散任务或让主角自行推断。任务卡之外的系统人格对白每次只回答眼前一个问题，或只给一条规则/一个可执行提示；禁止客服腔和后台流程腔，不要用任务阶段、状态核验、额度或权限解锁等后台字段播报剧情结果，可以偏心、吐槽，但不能替主角办事。
21. **关键首笔交付不能跳切**：若首笔交付、首次安装或第一次兑现承担本章闭环，不得只写“花了一个多小时”后直接宣布完成。至少保留一个真实阻力或调整、一次可见的测试结果和人物反应；只写决定成败的一两个场景拍，不展开材料、接线或操作步骤，不写施工教程。
22. **监管/谈判对白必须留缺口**：监管、授权或谈判对白不得连续三句完整讲齐时限、权限、责任；三项说齐前至少出现一次真正的插话、漏答或反问。只塞一个抬眼、停顿动作不算打断；正式任务卡的紧凑列项不受此条限制。
...
7. 若保留“呱，……”这一热梗，只能让角色直接开口，逗号后接一条完整吐槽，并保留同席人物的自然席间反应；不得改成“呱了一声”“怪叫一声”或任何拟声动作。不要求第二句续梗，生硬就整句删除。
8. 区分正式任务卡与系统人格对白：任务卡/结算卡允许紧凑列出已经确定的目标、时限、奖励并保留既定数字，不得隐藏数字或让主角自行推断；人格对白每次只回答眼前一个问题，禁客服腔和后台流程腔，也不要用任务阶段、状态核验、额度或权限解锁等后台字段播报剧情结果。
9. 若关键首笔交付、首次安装或第一次兑现承担本章闭环，压缩后仍须保留一个真实阻力或调整、一次测试结果和人物反应；用一两个决定成败的场景拍完成，不写施工教程，不能只剩“花了一个多小时”。
10. 监管、授权或谈判对白不得连续三句完整讲齐时限、权限、责任；三项说齐前必须有一次真正的插话、漏答或反问，不能用无功能动作冒充打断。正式任务卡的紧凑列项除外。
...
func protectedRuleRhymeAnchor(paragraph string) string {
	if !strings.Contains(paragraph, "儿歌") && !strings.Contains(paragraph, "童谣") && !strings.Contains(paragraph, "唱") {
		return ""
	}
	if strings.Count(paragraph, "；") < 2 && strings.Count(paragraph, "，") < 4 {
		return ""
	}
	runeCount := len([]rune(paragraph))
	if runeCount < 24 || runeCount > 500 {
		return ""
	}
...
func rewriteLengthInstruction(bounds rewritePatchBounds, retryFeedback string) string {
	...
	b.WriteString("- 初稿和压缩重试都不得牺牲适用的人味硬约束：正式任务卡保留既定目标/时限/奖励和数字；人格对白每次只答眼前一个问题；关键首笔交付保留真实阻力或调整、测试结果、人物反应；监管/谈判三项口径以插话、漏答或反问打断；若使用“呱，……”则保留完整吐槽和席间反应，不要求第二句续梗，生硬时整句删除。\n")
...
func validateRewritePreflight(st *store.Store, chapter int, rewritten string, includeYellowAIVoice ...bool) error {
	...
	failures = append(failures, toolspkg.ProjectContaminationViolations(st, rewritten)...)
