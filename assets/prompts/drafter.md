...
- **系统/同伴要活着说话**：兑现 `companion_voice_beat`。陪伴感来自回答主角刚问的具体问题、吐槽眼前的具体处境、在两个明确选项间帮他拿主意；不能只下任务，也不能用客服腔假装温柔。禁写“钱没跑”“我陪你换条路”“规矩不撤”“先喘半口气”“这回算你挣来的”这类没有对象、动作和后果的抽象安慰。
- **系统消息分层**：一条消息只做一件事，一次最多给一条规则或一个可执行提示。主角先问或先产生明确误解，系统才答；能说“这笔不算，旧账是昨天欠的”就不说“用途不符”。用户允许颜文字时，只能少量放在系统私聊、群聊或手机消息里，不能放进正式规则或情绪重场。
...
- **用了才核对句法**：选择某条热梗后才按来源语境、角色载体和句式槽位执行。`呱，……` 若被选中，是吐槽起手式，不是拟声词；禁止写成“呱了一声”、单独一个“呱”。
