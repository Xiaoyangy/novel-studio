...
1. 调用一次 `novel_context(chapter=N, profile="draft")`。必须确认 `chapter_world_simulation.status=ready`，以 `render_packet` 为正文唯一素材入口；`chapter_plan` 只用于标题、范围和禁区。读懂 `mandatory_beats`、`candidate_beats`、`protagonist_projection`、对白场景、人物声口、娱乐计划、章末契约以及五条渲染 policy。
2. 严守渲染边界：
