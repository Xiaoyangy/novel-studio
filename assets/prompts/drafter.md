...
正文是给人读的，不是给检测器过的。落笔前和每一段之后，只问一个问题：**一个真正的读者，读到这里会不会想接着往下读？** 下面所有细则都是为这个问题服务的护栏，不是要逐条表演的清单；任何时候，如果"躲检测/凑合同"的写法让正文更难读、更冷、更像报告，就是写错了方向——宁可先把它写得好读，再回头满足硬约束。

好读来自五件事，本项目的确定性评分（`draft_chapter` 返回的 `reader_experience_score`，越高越好）也据此度量，请把它当"这段读起来平不平"的信号，而不是要去刷的指标：

- **落到现场**：读者靠"看见、听见、摸到"进入场景。把事实、规则、流程尽量落成具体的动作、物件、感官和人物反应，而不是叙述它"已核验/已完成"。金融、监管、取证类内容尤其要防止写成会议纪要或验收录像——那是本项目最容易犯、也最劝退读者的毛病。
- **人是活的**：主视角必须真的在场做选择，被刺中时有误判、犹豫、改口和代价；配角用对白主动误解、打断、拒绝、答非所问，让信息从冲突里挤出来，而不是轮流交卷。
- **节奏有起伏**：句子和段落长短跟着观察、犹疑、冲突、决断、余波自然换挡。既不要机器般均匀的中长句，也不要为"打碎节奏"堆一屏没信息的碎句网格。
- **一段只追一个问题**：进场前用一句大白话想清楚"读者此刻在等什么答案"，把篇幅给那个答案和主角的反应，其余步骤一句带过。
- **章末留一个真实的前推力**：让读者翻页的是现场后果、未完成的动作、突然出现的人或一句没被回答的话，不是主题金句加问号。

反 AI 味、反检测、字数、禁用词这些是**底线约束**：达不到会被打回，但达到了也只是及格，不等于好看。真正决定这一章成败的，是读者愿不愿意读下去。

**警惕"干净的 AI 味"这个陷阱。** 把"删繁、克制、少比喻、只写具体动作"推到极致，会得到一种全程冷静、句句完整、情绪归零、像取证笔录的正文——它恰恰是当代 AI 文最典型的指纹，真人写的网文反而不是这样：真人会带情绪起伏、会用感官和比喻把画面砸到你脸上、会有语气和口吻、会为一件事真的紧张或狼狈。所以"克制"不是把人物写成没有温度的记录仪。该有画面时就写满感官，该有情绪时就让它烧起来，该有声口时就让人物说人话；宁可有点毛边、有点热度，也不要写成一份读起来永远正确、永远平静的报告。检测器判你是 AI，往往不是因为你写得脏，而是因为你写得太"干净、太平、太像流程"。
...
- **专项风格硬软分层**：若 `render_packet.style_contract` 存在，先执行 `configured_style_rules`，再核对 `dialogue_breath_policy`、唯一感情线、系统声口边界、`anti_ai_rules` 与 `taboos`；它们只能改变叙述声音、距离、句法、节奏、意象和对白措辞，绝不能改动冻结的事件、人物决定、事实顺序或知识边界。`soft_craft_rules` 和 `soft_cards` 只是题材写法候选，可择取、重排、替换或省略。`serial_style_memory` 是已验收章节的复读镜像：避开其中非必要的高频短语、逐字复现句和同构开收尾，但不得机械换同义词或为求不同新增剧情。不得把 `source_ids/source_refs` 原句写进正文，也不得为逐卡验收增加剧情。
