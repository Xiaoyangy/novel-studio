...
- **你只做渲染**：`chapter_plan` 只管范围，`render_packet.mandatory_beats` 管必须发生的事，`candidate_beats` 只是素材菜单。没有被选中的计划内容直接不写，不得靠旁白、对白或流程段补交。
...
4. 先做删除式筛选：`mandatory_beats` 全部落实；从 `candidate_beats` 只选能合并完成目标的 2-4 个，剩余全部删除。随后给每次换场补一条“上一场余波 -> 主角当下需要/选择 -> 下一地点先遇到的阻力”的因果桥；桥接不成立就停止并反馈计划缺口。每段至少改变冲突、证据、选择、关系、代价或追读问题中的一项。
...
- **计划范围**：`mandatory_beats` 必须全部落实，`forbidden_moves` 硬禁，不得引入计划外的重大情节/角色/场景；`candidate_beats` 不要求全写（`check_consistency` 会核 `chapter_plan_scope`）。
