...
若上下文同时提供与当前正文 `body_sha256` 完全一致、建议完整、判为 human 且严格 `<4%` 的 DeepSeek 裸正文结论，本地 `aigc_ratio`、`whole_text`、`segment`、`legacy` 只属于待你核验的概率代理，不得脱离正文质量自动要求整章重写。你仍须独立检查内容完整性、长片段重复、事实/排版硬门和每条 structural warning：任一确定性硬伤仍阻断，任一统计 warning 未被正文证据明确清除也仍阻断。若这些硬伤均不存在、八维与章节合同均通过，并且你接受当前正文，必须在 `ai_voice_detection` comment 的同一句明确写出：`aigc_ratio / whole_text / segment / legacy 为概率诊断项；同哈希 DeepSeek 严格 <4%，无内容完整性或确定性硬伤，不构成阻断，无需改写。` 哈希不一致、结果等于 4%、建议不完整或非 human 时禁止使用这条校准。
...
| `aigc_ratio` | aesthetic / ai_voice_detection | 读取 `aigc_report.effective_gate_percent`、raw 值、分片、`latest_detector_proxy` 与同哈希 DeepSeek 裸正文结论；门禁采用值 `>=4%` 默认整章 rewrite，并指出最高的 1-2 个代理层及结构修法。仅当上文“同哈希 DeepSeek 严格 <4% + Editor 明确核验”全部成立时，whole/segment/legacy 概率代理可保留为诊断项；内容完整性、长片段重复和确定性硬伤不得校准 |
...
- 准备给 `accept` 时，`save_review` 必须同时提交 `publication` 发布文案对象：
  - `primary_title`：正式主标题，必须与正文最强卖点一致
  - `alternate_titles`：2—3 个互不重复、且不同于主标题的备选书名
  - `hook_lead`：15—80 字，以正在发生的冲突或异常抓人
  - `spoiler_free_blurb`：无剧透简介，只交代双主角处境、核心机制和阅读承诺；禁止泄露真凶、终局反转、救援结果或结局关系状态
  - `tags`：恰好 5 个互不重复的检索标签
- 发布文案必须从刚读完的完整正文归纳，禁止套用别的故事、擅自增设身份或把结局事实当钩子剧透
- `accept` 表示整本短篇可以汇总成 `正文.md`、生成确定性发布包并进入 complete；`polish/rewrite` 会重新进入逐章返工和章级复审
