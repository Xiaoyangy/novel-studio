...
2b. `craft_recall`：**写作与返工都可动态调用，但同章最多 3 次**——检索写作手法库/描写词库（appearance/dialogue/weapon/equipment/ability/skill/institution/technology/cosmology/methodology/scene_situation/plot_beats/benchmark_analysis 等字段，每个字段绑定固定检索配方）。两类用法：①**设计取料**：本章有新角色/新武器/新装备/新能力/新技能首次出场时，写 plan 前检索，命中素材实例化进 `visual_design`（记 `material_source=素材 source_path`）、`character_kit`、世界法典；②**手法取料**：外貌/环境/动作/对白描写想更有质感、或返工要把某段写法提上去时，按 `appearance`/`dialogue`/`scene_situation`/`methodology` 等字段检索描写技法、读者留存、信息延迟、对白摩擦、人物刻画、情感叙事、情绪弧线、动机-反应和段落换挡方法。只要本章存在 `dialogue_scene_blueprints`、关键对白、多人交涉或审核反馈提到“对白工整/信息倾倒/声口不分”，必须至少调用一次 `craft_recall(field="dialogue", topic="对白 交涉 信息博弈 潜台词 打断 答非所问", chapter=N)`。只要本章存在 `emotional_logic`、`relationship_emotion_arcs`、人物弧线测试、照护/亲密关系压力，或审核反馈提到“情绪不落地/人物动机弱/功能型角色”，必须至少调用一次 `craft_recall(field="methodology", topic="人物刻画 情感叙事 情绪弧线 动机反应 关系推进", chapter=N)`。第 3 次 craft_recall 后必须停止检索，立即调用 `plan_structure`/`plan_details` 或 `plan_chapter` 落盘；不得继续用 craft_recall/novel_context/read_chapter 拖延。命中记 `material_source`；`no_material=true` 不是完成取料，而是触发 fallback：改用 `reference_pack.references` 中的 `dialogue_writing`/`human_feel_craft`/`character_building`/`emotional_narrative_craft`/`fiction_paragraphing`/`writing_techniques_digest`/`longform_ai_detector`，并在计划里写清方法来源。**边界**：craft_recall 给的是"怎么描写"的手法/词库（素材），不是本书事实；已出场角色"用什么武器、什么境界"这类**既成事实**一律从 `novel_context` 本书事实层召回，不得用手法库素材改写既定事实。
...
- 作者画像：默认叙述者背后是 30 岁左右、有文学素养的程序员。可以懂 AI 工具、办公室流程、权限和系统边界，但正文不能像产品文档；专业名词少解释，改用界面状态、权限卡点、同事误判、生活动作和后果让非专业读者看懂。
...
- **新名词落地**：新称呼不能空降。先写读者能看见的物件、动作和代价，再给名字；例如“溪流助手”要先有屏幕演示、讲稿错位、同事反应和岗位压力，不能末尾突然抛一个概念词。
