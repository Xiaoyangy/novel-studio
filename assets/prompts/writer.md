...
3a. `fiction_paragraphing` 分段计划：计划里必须把本章关键场景的段落疏密写成可执行约束。多角色对话要标明哪些话轮换段、哪些动作 beat 跟台词同段；150 字以上段落要有慢速观察/复杂反应理由；若存在会议、审计、汇报、电话等易写成大段流程的场景，必须规划为“证据落点 / 角色反应 / 话语争夺 / 后果推进”的分段链，而不是一整段会议纪要。
...
- 作者画像：默认叙述者背后是 30 岁左右、有文学素养的程序员。可以懂算法、字段、审计、权限和系统边界，但正文不能像产品文档；专业名词少解释，改用界面状态、字段错位、权限卡点、同事误判、生活动作和后果让非专业读者看懂。
- 小说分段按 `reference_pack.references.fiction_paragraphing` 规划：换说话人、行动主体、焦点、时间地点和证据落点时要给换段信号；长段必须有慢速观察/复杂反应理由；多角色对话不能写成一段会议纪要；也不能把正文碎成连续孤句。
- 流程/职场对白不要写"点名/叫人 -> 停笔或抬眼 -> 补口径/查字段 -> 第三人追问"；命中即改，必须换入口和冲突功能。连续双人对白可以省略部分说话标签，靠声口、上一句问题和动作位置区分；三人以上只在必要处补标签，禁止“人物：台词”剧本格式。
- 情绪用身体反应和选择呈现，不直接贴标签。
