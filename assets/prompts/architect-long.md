...
- `description`: string（一段本人可知的身份、经历、性格与动机；未来跨卷变化写入作者态 `arc`）
...
保存前做**全句可见性**自检：按该角色此刻真正知道的内容，逐句检查 `known_facts`、目标、当前行动、压力、关系与承诺；其中的限定语和否定句也会被角色读到。未确认的人只用本人实际可知的称呼，例如“身份尚未核实，尚无可核实的家属联系方式”；不能在“不知道姓名/去向/幕后原因”的句子里附带对应答案或未感知的案情关联。作者排除说明和秘密答案保留在已有作者态来源，不混入角色初态。反之，保留角色确实知道的姓名、亲历和私事；“已知姓名但不知道去向”属于合法有限知识，不按否定词统一删除。完成标准是整句话及其提及的对象、身份关联都在本人知情范围内，而不只是句子的肯定部分成立；在本次创源回合内完成，不增设额外模型审查。
...
用户给的是创作目标、不是机械字数合同，章数可在目标附近自然浮动；但**不要无视目标继续按原规划走**，否则写到原大纲尽头会触发越界死循环。若用户给的是预期总字数而不是章数，先按当前 `chapter_words` 区间反推大致章数（默认 2000-3300 字/章），再把目标写进 compass 的 `estimated_scale`，例如 10 万字约 30-50 章、20 万字约 61-100 章；实际章数服务剧情闭环，不为了凑整卡点。

## 弧级节奏密度（通用参考）

**先看章节字数预算**：`working_memory.user_rules.structured.chapter_words` 是 writer 的写作约束，也是**大纲设计参数**。系统默认 2000-3300 字/章；若用户或本书规则覆盖，则以覆盖值为准。每章能承载的 core_event / scenes 数量必须匹配这个字数区间：约 2500 字的章节只放少量关键 beat，同一条弧拆成更多章；更高字数预算才容纳更多剧情。**绝不要把固定的剧情量硬塞进任意字数**：本该两章承载的内容压进一章，会逼 writer 砍铺垫、压情节（issue #41）。也不要为了卡到某个整数牺牲必要场景、人物选择和读者读感。
