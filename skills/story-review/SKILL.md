...
| 人工感正向标尺 | `story-review/references/human-feel-craft.md` |
| refer 写作技巧总纲 | `story-review/references/refer-writing-techniques-digest.md` |
...
- 对话质量：是否有潜台词、信息控制、角色差异；说明书式对话至少 S2。整场对话结束时没有任何价值极性翻转（信任/安全/希望/亲密/控制权原地不动）、或强弱格局从头到尾没易手一次的关键对话场，按 S3 报；同一角色同一说话战术连用三回合不换按 S3。
- 设定一致性：不违背已写规则、时间线、角色属性；明确事实冲突通常 S1。
- 文字自然度：具体、可感、动作承载信息；AI 腔、陈词滥调、总结体按影响定 S2/S3。
- 人工感承载：正文是否有现场异常、物件/痕迹回扣、主观误判、短对话/动作拍、现实支架和可复核因果链；只靠情绪标签、解释和空泛总结推进时按 prose/causal 问题处理。
- refer 技巧执行：是否有前台故事、目标/阻力/失败代价/新增信息、钩子接力、事件铺垫/过程/余波和过渡章期待铺垫；缺失时按 structure / causal / prose 分级。
...
5. **形成审查基准包摘要**：把已加载的文件内容或内置 fallback 摘要压缩为 5-12 条审查标准，后续 solo 和子 Agent 都必须使用这份摘要。若 `story-review/references/human-feel-craft.md` 可读，摘要必须包含人工感正向标尺：现场异常、物件回扣、主观误判、短对话/动作拍、现实支架和可复核因果链；不可读时使用内置人工感 fallback。若 `story-review/references/refer-writing-techniques-digest.md` 可读，摘要必须包含 refer 技巧标尺：前台故事、目标/阻力/代价/新增信息、钩子接力、事件铺垫/过程/余波、过渡章期待铺垫和标点功能；不可读时使用内置 refer fallback。
...
- 提示指令：
  ...
  7. 好感度进度是否可感知？
  8. 对话六症状（可选读 `story-setup/references/agent-references/dialogue-mastery.md` 自查项）：① 机械对话/问答式/句间无情绪承接；② 角色当「科普嘴」整段讲设定原理(Gate G 同样管台词)；③ 说话不分场合(高压/生死 beat 的玩笑、口头梗、插科打诨出戏)；④ 价值静止(整场对话结束时信任/安全/希望/亲密/控制权没有任何极性翻转，剧情和关系原地踏步)；⑤ 权力静止(强弱格局从头到尾没易手一次，弱方全程挨压或强方全程被打脸)；⑥ 战术复读(同一角色同一说话策略连用三回合不换：连威胁三次、连求三次、连讽三次)。命中按 S2/S3 报具体引用+改法。
...
- 提示指令：
  ...
  人工感标尺摘要：{从 human-feel-craft 或内置 fallback 提取，必须内联}
  refer 技巧标尺摘要：{从 refer-writing-techniques-digest 或内置 fallback 提取，必须内联}
  ...
  7. 身体部位同一词是否超 5 次？
  8. 是否有人工感承载：现场异常、物件/痕迹回扣、主观误判、短对话/动作拍、现实支架和可复核因果链是否缺失？
  9. 是否执行 refer 技巧：目标/阻力/代价/新增信息是否清楚，核心事件是否有铺垫/过程/余波，过渡段是否有期待铺垫，钩子是否接力？
  10. AI味分级（轻度/中度/重度）及证据。
...
4. 人工感承载检查（优先读取 `story-review/references/human-feel-craft.md`，不可读时使用内置人工感 fallback：现场异常、物件回扣、主观误判、短对话/动作拍、现实支架、可复核因果链）。
5. refer 技巧检查（优先读取 `story-review/references/refer-writing-techniques-digest.md`，不可读时使用内置 refer fallback：前台故事、目标/阻力/代价/新增信息、钩子接力、事件铺垫/过程/余波、标点功能）。
6. 通用网文内容评分（优先读取 `story-review/references/quality-rubric.md`，不可读时使用内置通用网文内容 rubric）。
...
### 人工感承载
- 现场异常 / 人物动作 / 主观偏差：通过/不通过；证据：...
- 物件或痕迹回扣：通过/不通过；证据：...
- 对话和动作拍是否替代解释：通过/不通过；证据：...
- 误会、反转、和解或危机是否有前文可复核证据：通过/不通过；证据：...
- 现实支架是否支撑人物选择：通过/不通过；证据：...
