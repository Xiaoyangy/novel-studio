...
| 直接开写 | novel-pipeline / novel-write（story-long-write 只作方法参考） | `novel-studio --pipeline --prompt-file <需求文件>` |
| 更适合短篇 | story-short-scan | `/story-short-scan` |

> **选题决策.md 交接**：Phase 4 产出的 `选题决策.md` 写在扫榜输出目录（扫榜常在没有小说项目时进行）。开书时把它搬到小说项目根目录，并把摘要写入 pipeline 需求文件；拆文（story-long-analyze）会在汇总报告产出后回填对应选题的"能爆的原因"。
