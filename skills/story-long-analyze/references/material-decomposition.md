...
优先使用 chapter-extractor agent 并行处理。每章 spawn 一个 agent，每批最多 5 个并发。Codex 调用方必须显式写 `spawn N agents` 或 `并行拆给 N 个 agent`（N ≤ 5），否则不要假定会自动开 subagent。
Agent 不可用时退回主线程串行处理。
...
> 模板与示例见 [output-templates.md](output-templates.md) 「Stage 4 设定+关系」节。下游同步检测到 `设定/世界观/` 子目录已存在时直接 pass-through，不再 re-split。
...
Stage 2 使用 chapter-extractor agent 并行处理（每章一个 agent，每批最多 5 个），不分块。
