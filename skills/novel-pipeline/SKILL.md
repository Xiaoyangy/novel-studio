---
name: novel-pipeline
description: "把各功能串成一条可恢复的流水线：（共创→）写作→评审→重写→导出，按阶段执行、断点续跑。触发：「一条龙写完并导出」「从头跑完整个流程」「中断后接着跑」，想要无人值守端到端、且能中断恢复时使用。"
---
...
`cocreate`（可选，多轮澄清）→ `write`（创作）→ `review`（评审）→ `rewrite`（重写）→ `export`（导出）

默认序列：`write,review,rewrite,export`（假设已有创作指令）。
...
| `--stages a,b,c` | `write,review,rewrite,export` | 阶段子集与顺序 |
| `--export-out <path>` | `{novelDir}/{书名}.txt` | `export` 阶段输出路径（后缀决定 TXT/EPUB） |
...
- 已标记完成的阶段重跑时会先复核证据；若产物被删、checkpoint 缺失或导出文件不存在，会清掉完成标记并重跑该阶段
- 证据口径：`write` 看 `progress.phase=complete`、章节文件和 `commit_chapter` checkpoint；`review` 看 `reviews/*.md` 与 `meta/review-summary.md`；`rewrite` 看章节文件和 `.pre-rewrite.md` 备份；`export` 看非空导出文件
