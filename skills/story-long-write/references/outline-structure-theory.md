...
1. **选段**：在主对标里圈出本卷要对标的一级结构序列。优先读 `剧情/节奏.md`（爆发密度 / 爽点循环）；没有就用 `章节/*_摘要.md`（每个情节点带「类型」「基调」标签）+ `拆文报告.md`（剧情条的章节范围 / 结构分布）——后者足够还原，旧deconstruction-library常无 `节奏.md`。本卷对标几条一级结构 ≈ 本卷字数 ÷ 1.5-3 万。
