...
4. 若任务已经开始，优先读执行目录里的 `.skill-context/story-short-scan.md`，恢复已读文件、阶段、路径、硬约束和下一步。

## 压缩恢复规则

- 主会话只保留轻量状态：任务名、当前阶段、当前产物路径、已读资料清单、下一步。
- 长正文、拆文结果、审核证据、平台资料和脚本输出必须落盘，恢复时按路径读取。
- 每次完成阶段、写完章节、完成审核或切换 agent 前，更新 `.skill-context/story-short-scan.md`。
- 如果上下文压缩后缺少关键信息，不凭记忆续写；先回读 `.skill-context/story-short-scan.md` 和相关产物。

## 本 skill 的读取重点

- 必读补充文件：`references/real-market-data.md`、`scripts/cdp-utils.js`、`scripts/dz-browse-scraper.js`、`scripts/heiyan-booklist-scraper.js`。
...
```markdown
# story-short-scan execution context
- task: <用户任务>
- stage: <当前阶段>
- inputs: <关键输入路径>
- outputs: <关键输出路径>
