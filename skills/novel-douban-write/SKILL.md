...
```bash
novel-studio --check
novel-studio --pipeline --prompt-file ./豆瓣长篇需求.md --stages write,review,rewrite,export
```
...
`novel-check` -> `novel-cocreate` 或 prompt 文件 -> `novel-pipeline(write,review,rewrite,export)` -> 最终人工/agent 检查包装材料。
