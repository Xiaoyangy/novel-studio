...
> Agent 兼容性：检查专业 agent 是否可用时，按 `.claude/agents/{agent}.md` → `.opencode/agents/{agent}.md` → `.codex/agents/{agent}.toml` 的顺序查找。Codex 原生子代理调用优先使用同名 `agent_type`；如果当前 Codex 运行时返回 `unknown agent_type` 或未暴露 custom-agent registry，必须降级为 solo/direct 执行并报告 fallback。Claude/OpenCode 兼容面保留 `subagent_type`。Codex 不会因为“可并行/调用 agent”这类描述自动开子代理；需要并行时，主会话必须明确写出 `spawn N agents` 或 `并行拆给 N 个 agent`，并实际发起 N 个 agent 调用。

## 多小说调度与落盘规则

如果用户一次给出多个不同题材、书名或核心设定，按每本书拆成独立任务；能调用子代理时，为不同小说并发启动独立 agent。**单批最多 5 个 agent**，`N = min(待处理小说数, 5)`；待处理小说超过 5 本时，其余进入 pending 队列，等本批全部完成后再开下一批。Codex 执行并行批次前必须显式写 `spawn {N} agents` 或 `并行拆给 {N} 个 agent`，否则不要假定 Codex 会自动开 subagent。每个 agent 只处理一本书，只读取该书目录、设定、小节大纲和当前写作摘要；主会话只保留调度表（running/pending/done），避免把多本正文塞进同一上下文。
...
- **拒绝模板扩写感**：公式只决定情绪顺序和功能位，不能把小节大纲逐条平均扩成正文。每个小节必须有本故事独有的物件、动作打断、错字/口癖/时间戳/金额/私人物件等具体锚点；段长、对话密度、情绪强弱要不均匀。禁止每节都按“冲突开场→解释回忆→台词爆发→章末金句”的同构段式收口。
- **人工感来自现场承载**：加载 `references/human-feel-craft.md`，把《同桌是只假装高冷的猫》80% 人工度样本文的方法迁移为本篇的物件回扣、主观误判、短对话/动作拍和现实支架。短篇可以直写强情绪，但情绪后面必须接本场景特有的动作、物件或选择。
- **refer 写作技巧总纲**：加载 `references/refer-writing-techniques-digest.md`，把 19 篇写作技巧压缩成短篇的设计和审核问题：前台故事是否先行、每节是否有目标/阻力/代价/新增信息、核心事件是否有铺垫/过程/余波、钩子是否接力、标点是否承担声口和情绪功能。
...
> 如果用户有参考小说，先用 `/story-short-analyze` 拆解。默认输出存入项目根目录 `deconstruction-library/{书名}/`；如用户指定当前短篇引用目录，则可输出/同步到 `{短篇标题}/对标/{书名}/`。写作时会自动查找并读取这些拆文结果，不需要用户手动复制到 prompt。

#### 对标上下文加载

> **deconstruction-library/对标关系**：`deconstruction-library/` = analyze skill 的原始产出（数据源），位于项目根目录。`对标/` = 当前短篇的引用视图，位于 `{短篇标题}/对标/`。短篇写作优先读取 `{短篇标题}/对标/{书名}/`，不存在则回退项目根 `deconstruction-library/{书名}/`，再回退 `{短篇标题}/deconstruction-library/{书名}/`（兼容旧结构）。
...
如果工作目录下存在 `对标/` 或项目根存在 `deconstruction-library/`，或用户提到参考小说：
...
**拆文结果自动使用规则**：执行写作前必须按“对标上下文加载”顺序扫描 `{短篇标题}/对标/{书名}/`、项目根 `deconstruction-library/{书名}/`、`{短篇标题}/deconstruction-library/{书名}/`。找到拆文报告时，把“结构/情绪/反转/写作手法”作为技法参考；找到结构化子目录时，按当前小节目标检索最相关模块。

> 术语说明：Phase 3 按「段」划分叙事结构（开头段/铺垫段/升级段/反转段/结尾段），每段包含若干「小节」（数字编号的 beat）。「场景」指写作时的具体画面。

**写前准备**（每个场景写前执行召回、人工感锚点、refer 技巧核对和指令确认，是核心方法的落地：确认情绪目标 → 召回技法模块 → 校验单节任务）：
- **步骤 1：记忆+召回**：① 本场景目标情绪词？② 借鉴哪个参考文件的哪个技法？③ 具体用在哪个段落？答不出 → 先回读参考再动笔。如有 `对标/` 或 `deconstruction-library/` 结构化产出，按“对标上下文加载”规则检索与当前场景最相关的结构/情绪/反转/写作手法模块作为参考，并写入“拆文召回摘要”
  - **多对标书时**：参 `references/cross-book-recall.md`，副对标/参考对标按阶段预算进入"副对标召回摘要"；正文只传摘要，不传副书文风或原文
- **步骤 1.5：人工感锚点**：按 `references/human-feel-craft.md` 给本场景写 1 行锚点：本节出现哪个物件/痕迹、它的新信息是什么、角色会在哪句话或动作里误判/改口、后文如何回扣。没有锚点时先补设计再写。
- **步骤 1.6：refer 技巧核对**：按 `references/refer-writing-techniques-digest.md` 用一句话回答本节目标、阻力、失败代价和新增信息；标出本节的铺垫/过程/余波以及章末或节末的未完成期待。答不出时先改小节大纲。
...
- [ ] 身体部位同一词全文 ≤ 5 次
- [ ] 「像」≤ 10 处
- [ ] 每节至少有 1 个物件/痕迹/现实支架承担新信息；反转、误会或和解有前文可复核证据（按 `references/human-feel-craft.md` 自检）
- [ ] 每节能回答目标/阻力/失败代价/新增信息；核心事件有铺垫/过程/余波；标点按人物声口和场景功能使用（按 `references/refer-writing-techniques-digest.md` 自检）
- [ ] `node scripts/check-ai-patterns.js --check {短篇标题}/第01章_正文.md` 无高危 AI 对比句式与破折号命中（碎句号/长段落按提示处理）
- [ ] `node scripts/check-degeneration.js --check {短篇标题}/第01章_正文.md` 无 blocking 退化命中（复读/截断/工程词泄漏）
...
重点：开头钩子、情绪曲线、反转铺垫、每句话价值、格式规范、人工感承载、AI 腔排查。人工感按 `references/human-feel-craft.md` 检查：现场异常、物件回扣、主观误判、短对话/动作拍、现实支架和可复核因果链缺一项时，优先补场景承载而不是替换形容词。文件模式必须先运行 `node scripts/check-ai-patterns.js --check {短篇标题}/第01章_正文.md`（报告 AI 对比句式 + 破折号按功能改写 + 碎句号 + 长段落；前两类复扫到 0，后两类按情况改），再运行 `node scripts/normalize-punctuation.js {短篇标题}/第01章_正文.md` 机械兜底标点。另跑 `node scripts/check-degeneration.js --check {短篇标题}/第01章_正文.md` 报告模型退化（复读/截断/工程词泄漏）；blocking 命中说明该段要重新生成，不是改写。

#### Agent 调用：narrative-writer（去AI味）+ consistency-checker

精修阶段，如果项目已部署对应 agent，可 spawn：
- `Agent(subagent_type: "narrative-writer", prompt: "项目目录：{dir}\n任务描述：去AI味+格式检查\n检查范围：{正文文件}\n删除优先：每条 AI 味项先判能否删除——删后不丢伏笔/钩子/角色/情节/必要信息的直接删，会丢才润色（删除受比例上限与字数下限约束，跌破下限改降AI重写）\n必须检查：先否定再肯定的翻转句式；发现后直接改成后项或动作细节；按 human-feel-craft 检查物件回扣、主观误判、短对话/动作拍和现实支架是否缺失")` — 执行去AI味（7 Gate）、人工感承载和格式合规检查
...
| [references/human-feel-craft.md](references/human-feel-craft.md) | 写作和精修全程参考（同桌样本文人工感：物件回扣、主观误判、短对话、现实支架） |
| [references/refer-writing-techniques-digest.md](references/refer-writing-techniques-digest.md) | 构思、写前准备和精修全程参考（19 篇 refer 写作技巧压缩：前台故事、单节目标/阻力/代价/新增信息、事件余波、标点功能） |
...
| 人工感 / 语意换挡 | **`references/human-feel-craft.md`**（同桌样本文：物件回扣、主观误判、短对话/动作拍、现实支架） | `references/short-craft.md`（情绪词后接场景动作）· `references/dialogue-mastery.md`（对话潜台词）· `references/short-deslop.md`（去 AI 腔） |
| 短篇设计/单节/标点总纲 | **`references/refer-writing-techniques-digest.md`**（19 篇 refer 写作技巧逐篇压缩：前台故事、目标/阻力/代价/新增信息、钩子接力、事件铺垫/过程/余波、标点功能） | `references/short-craft.md`（短篇正文底座）· `references/hooks-chapter.md`（钩子）· `references/dialogue-mastery.md`（对白声口） |
