...
2. **主对标书选择**：
   ...
   - 若 `对标/` 无子目录，继续向上找工作区根下的 `deconstruction-library/*/`；若仍无可用目录 → 返回 `gaps.no_benchmark: true`，`results` 置空，**不报错、不继续读文风**
3. **对标书路径查找**：优先 `{项目}/对标/{书名}/`，回退 `deconstruction-library/{书名}/`（向上找到工作区根，再下钻deconstruction-library）
4. **判定对标契约版本（先于回退）**：
