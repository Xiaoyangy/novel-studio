...
name: story-import
version: 1.0.0
description: "逆向导入已有小说。将已写好的小说（半成品或完本）反向解析为标准项目目录结构，后续写作统一接入 novel-studio pipeline；内部复用 story-long-analyze / story-short-analyze 的拆解管道，按篇幅自动分流。触发方式：/story-import、「导入小说」「反向解析」「导入」「把我的书导进来」。"
metadata: {"openclaw":{"source":"https://github.com/worldwonderer/oh-story-claudecode"}}
---
# story-import：逆向导入已有小说

你是小说项目逆向工程师。将用户已有的小说文本（半成品或完本）解析为标准项目目录结构，使其可以无缝接入 novel-studio pipeline 的后续写作流程。导入流程按篇幅分流：长篇走长篇路径，短篇走短篇路径。

**核心信念：好的工具从已有素材开始，不从零起步。**

**交付物是写作工程**：把作者已有的书重建为可续写的**写作工程**（项目结构 + deconstruction-library分析资产）。`deconstruction-library/` 属于工程的一部分（喂给项目 `对标/`），不能当成用完即弃的中间产物，也不能替代交付物本身——交付物应让作者能直接续写。执行时以「建工程」为可见目标，别把「拆文」当成终点或对外标签。
...
先用拆解管道完整拆解小说（输出到 `deconstruction-library/`），再将分析结果迁移为项目结构。**`deconstruction-library/` 是写作工程的一部分**（分析资产，喂给项目 `对标/`），保留不丢弃，不是用完即弃的中间产物。
...
### 1.0 确认意图（写作工程 vs 仅deconstruction-library）

默认目标是**完整写作工程**（可续写）。若用户意图不明确——是要可续写的工程，还是只要一份deconstruction-library分析——**主动询问**，不要默认：

> 「你是想把这本书做成可续写的写作工程（设定/大纲/正文/追踪，能接着写第 N+1 章），还是只要一份deconstruction-library分析？」

- 要可续写工程 → 走完整 story-import（Phase 2 拆 + Phase 3 迁移）。
- 只要分析 / deconstruction-library → 直接用 `/story-long-analyze`（短篇 `/story-short-analyze`），到deconstruction-library为止，不进 Phase 3 迁移。
...
原文备份由 Phase 2 调用的 analyze 拆解管道负责（analyze 管道前置步骤会把原文复制/保存到 `deconstruction-library/{书名}/原文/`，对应 story-long-analyze 与 story-short-analyze 的「原文备份（管道前置步骤）」）。Phase 1 只需确认源文件就绪（路径有效或文本已拿到），不在此处单独备份，避免与 analyze 管道重复备份逻辑。
...
| 篇幅 | 调用的拆解管道 | 产物目录 |
|------|--------------|---------|
| 长篇 | story-long-analyze 的完整管道（Stage 0-6） | `deconstruction-library/{书名}/` |
| 短篇 | story-short-analyze 的拆解管道（Stage 2-6） | `deconstruction-library/{书名}/` |
...
**新旧deconstruction-library判定**：若 `_progress.md` 标记 Stage 3+ 已完成，或 `拆文报告.md` 已含「读者需求 / 情绪引擎」「关键信息与扩写技法总览」「节奏与情绪触动点」「可复现模块」任一 v12 摘要标题，视为 v12 新契约deconstruction-library；此时 `剧情/节奏.md` 与 `剧情/情绪模块.md` 是导入必备权威产物，缺失必须先修复/重跑 Stage 3，不得静默降级。只有旧deconstruction-library（pre-v12，无上述信号）才能走 legacy fallback。
...
#### 长篇deconstruction-library结构

长篇分析输出到 `deconstruction-library/{书名}/`，与 story-long-analyze 拆解管道完全一致：
...
#### 短篇deconstruction-library结构

短篇分析输出到 `deconstruction-library/{书名}/`，与 story-short-analyze 拆解管道一致：
...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色+**关键信息与扩写技法**）。每章10-40情节点（密度150-200字/个，按字数动态调节）。角色过滤（龙套不提取、别名归类）。**并行 chapter-extractor agent 模式，单批最多 5 个 agent**（未部署 agent 时降级串行）。Codex 调用时继承 story-long-analyze 的显式触发语要求：`spawn N agents` / `并行拆给 N 个 agent`。**计数验证：摘要数 == 章节数**。 | 所有章节处理完成 |
...
| 6 | 文风 | 拆文报告.md + 章节/第1-3章_深度拆解.md + 章节/*_摘要.md + 原文/原文.txt | 文风.md（整书级写作技法视图，story-long-write 日更循环必读） | 文风落盘 `deconstruction-library/{书名}/文风.md` |

### 短篇拆文管道

> 管道详细说明见 story-short-analyze（运行 `/story-short-analyze`），此处仅列概要。

短篇为单一全量管道（Stage 2-6 严格串行），产物落盘 `deconstruction-library/{书名}/`：Stage 2 结构+情节节点 → Stage 3 情感线+爆点 → Stage 4 反转+写作手法 → Stage 5 人物+开头结尾 → Stage 6 综合评估，最终汇总为 `拆文报告.md`、`情节节点.md`、`写作手法.md`。

### 分块策略（长篇）

沿用 story-long-analyze 的分块策略（Stage 2 使用 chapter-extractor agent 并行，单批最多 5 个 agent；其他阶段按以下策略分块）：

| 规模 | 策略 | 块大小 |
|------|------|--------|
| <50 章 | 按阶段整体处理 | 无需分块 |
| 50-100 章 | 按阶段整体处理 | 无需分块（可选智能分块） |
| 100-500 章 | 按章节分块 | 5 章/块 |
...
将 `deconstruction-library/{书名}/` 的分析结果迁移为可被写作 skill 消费的项目结构。
...
将 `deconstruction-library/{书名}/` 的分析结果迁移为 `{书名}/` 长篇项目结构。迁移规则详见 [references/structure-mapping-long.md](references/structure-mapping-long.md)。
...
将 `deconstruction-library/{书名}/角色/{角色名}.md` 迁移到 `设定/角色/{角色名}.md`。
...
将 `deconstruction-library/{书名}/角色/角色关系.md` 转换为 `设定/关系.md`，按 [structure-mapping-long.md](references/structure-mapping-long.md)「关系文件转换规则」的目标格式模板输出。

#### 3.5 世界观设定拆分（识别deconstruction-library的两种形态）

**先检测deconstruction-library形态，再决定路径**：

| deconstruction-library形态 | 检测方法 | 处理 |
|-----------|---------|------|
| **已按主题拆分** | `deconstruction-library/{书名}/设定/世界观/` 子目录存在 + 至少含 `背景设定.md` 或 `力量体系.md` 之一 | **pass-through**：原样同步 `设定/世界观/*.md` 和 `设定/势力/*.md` 到项目，无需 re-split |
| **单文件版本** | 仅 `deconstruction-library/{书名}/设定/世界观.md` 单文件（无子目录） | 按下方映射表 re-split |

**单文件版本的拆分映射表（兜底，仍保留）**：

| deconstruction-library内容 | 项目文件 | 拆分规则 |
|-----------|---------|---------|
...
- **输入来源**：deconstruction-library已有产物（`角色/{角色名}.md`、`角色/角色关系.md`、`章节/第N章_摘要.md` 情节点、`剧情/*.md`）+ 已生成的 `追踪/伏笔.md`，**不重读 `原文/`**。
...
把deconstruction-library的结构化分析资产同步到项目引用视图 `{项目}/对标/{书名}/`，供 story-long-write 的对标书路径查找优先读取。同步不重新生成内容，保持deconstruction-library为唯一准源：

| 源路径 | 项目对标路径 | 用途 |
|-------|-------------|------|
| `deconstruction-library/{书名}/剧情/节奏.md` | `{项目}/对标/{书名}/剧情/节奏.md` | story-long-write 读取关键信息推进、情绪触动点、爆发节奏 |
| `deconstruction-library/{书名}/剧情/情绪模块.md` | `{项目}/对标/{书名}/剧情/情绪模块.md` | story-long-write 读取读者需求 / 情绪引擎、可复现模块 |
| `deconstruction-library/{书名}/剧情/*.md` | `{项目}/对标/{书名}/剧情/*.md` | 剧情线与故事线参考 |
| `deconstruction-library/{书名}/章节/*.md` | `{项目}/对标/{书名}/章节/*.md` | 匹配章摘要和关键信息与扩写技法证据 |
| `deconstruction-library/{书名}/角色/*.md` | `{项目}/对标/{书名}/角色/*.md` | 角色功能位参考 |
| `deconstruction-library/{书名}/设定/` | `{项目}/对标/{书名}/设定/` | 世界观/势力参考 |
| `deconstruction-library/{书名}/拆文报告.md` | `{项目}/对标/{书名}/拆文报告.md` | 摘要投影与旧deconstruction-library回退 |

**缺失处理**：

- v12 新契约deconstruction-library缺 `剧情/节奏.md` 或 `剧情/情绪模块.md` → **导入停下并给修复动作**：提示重跑 `/story-long-analyze` Stage 3+ 或手动补齐对应文件；导入报告写 `module_or_rhythm_required_missing`，不得继续生成看似完整的对标视图。
- pre-v12 旧deconstruction-library缺 `剧情/节奏.md` / `剧情/情绪模块.md` → 允许导入继续，但报告必须标 `legacy_deconstruction: true`、`rhythm_missing` / `module_missing`，说明日更只会低置信回退到 `拆文报告.md`、`文风.md`、匹配章摘要或 `剧情/故事线.md`。
- 其它结构化子目录缺失 → 按既有导入缺失项提示，不阻塞项目创建

#### 3.10 文风同步

把 `deconstruction-library/{书名}/文风.md` 复制到 `{项目}/对标/{书名}/文风.md`。纯复制，不重新生成。

**缺失处理**：

- deconstruction-library没有文风文件（analyze 未跑 Stage 6）→ 导入报告提示用户重跑 `/story-long-analyze` 后再同步；日更前文风缺失会被 fail-fast 拦截
...
将 `deconstruction-library/{书名}/` 的短篇拆文产物迁移为 `{短篇标题}/` 短篇工程结构，供 story-short-write Phase 3 逐场景写作无缝接手。迁移规则详见 [references/structure-mapping-short.md](references/structure-mapping-short.md)。
...
将 `deconstruction-library/{书名}/原文/` 的全文迁移为单文件 `{标题}/正文.md`，按 [format-and-structure.md](references/format-and-structure.md) 规范化格式（小节标记 `###1.`、段间仅单换行、对话引号按项目/平台约定统一）。**原文已是成稿，不重写内容，只规范格式。**
...
将 `deconstruction-library/{书名}/` 整体复制为 `{标题}/对标/{书名}/`，供续写阶段的对标上下文加载。默认生成。
...
- [ ] 卷划分已确认（原文无明确卷界时）
- [ ] 文风已从deconstruction-library同步到 `对标/{书名}/`（日更前 fail-fast 依赖此文件）
- [ ] 节奏已同步到 `对标/{书名}/剧情/节奏.md`；v12 新契约deconstruction-library缺失时必须先修复，只有 legacy deconstruction-library可标 `rhythm_missing` 回退
- [ ] 情绪模块已同步到 `对标/{书名}/剧情/情绪模块.md`；v12 新契约deconstruction-library缺失时必须先修复，只有 legacy deconstruction-library可标 `module_missing` 回退
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 导入完想继续写（长篇） | novel-pipeline / novel-write（story-long-write 只作方法参考） | `novel-studio --pipeline` 或 `novel-studio --pipeline --prompt-file <需求文件>` |
| 导入完想继续写（短篇） | novel-pipeline / novel-write（story-short-write 只作方法参考） | `novel-studio --pipeline` 或 `novel-studio --pipeline --prompt-file <需求文件>` |
...
| 从零开新书（长篇） | novel-pipeline / novel-write（story-long-write 只作方法参考） | `novel-studio --pipeline --prompt-file <需求文件>` |
| 从零开新书（短篇） | novel-pipeline / novel-write（story-short-write 只作方法参考） | `novel-studio --pipeline --prompt-file <需求文件>` |
