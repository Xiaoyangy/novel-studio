...
Phase 3-S 短篇结构迁移的详细映射规则。将 `deconstruction-library/{书名}/` 短篇拆文产物转换为 `{短篇标题}/` 短篇工程结构，供 `story-short-write` Phase 3 无缝接手续写。
...
| 拆书产物 | 短篇工程文件 | 转换方式 |
|---------|------------|---------|
| 原文全文（`deconstruction-library/{书名}/原文/`） | `{标题}/正文.md` | 单文件迁移，按 `format-and-structure.md` 规范化格式（小节标记 `###1.`、段间仅单换行、对话引号按项目/平台约定统一）；原文已是成稿，**不重写内容，只规范格式** |
...
| `deconstruction-library/{书名}/` 整体 | `{标题}/对标/{书名}/` | 可选：复制为引用视图，供续写阶段按需加载 |
...
将 `deconstruction-library/{书名}/` 整体复制为 `{标题}/对标/{书名}/`，供续写阶段的对标上下文加载：
