...
Phase 3-L 长篇结构迁移的详细映射规则和模板。将 `deconstruction-library/{书名}/` 分析结果转换为 `{书名}/` 长篇项目结构。
...
| deconstruction-library路径 | 项目路径 | 转换方式 |
|-----------|---------|---------|
...
## 基本信息
- 身份：{从deconstruction-library角色文件提取}
- 核心特质：{}
- 当前能力：{}
- 核心动机：{}
- 弱点/缺陷：{}
...
deconstruction-library格式（角色关系.md）→ 项目格式（设定/关系.md）：

```
deconstruction-library格式：
...
## 内容
{从deconstruction-library设定文件提取的具体内容}
```
...
- **输入来源**：deconstruction-library已有产物（角色档案、关系.md、章节摘要、剧情文件），不重读 `原文/`；
...
长篇迁移必须把deconstruction-library关键分析资产同步到项目侧 `对标/{书名}/`，使 story-long-write 优先读项目引用视图，旧工程仍可回退到根目录 `deconstruction-library/{书名}/`。

| 源路径 | 目标路径 | 同步语义 |
|-------|---------|----------|
| `deconstruction-library/{书名}/剧情/节奏.md` | `{项目}/对标/{书名}/剧情/节奏.md` | 节奏已同步后，日更用它选择 `rhythm_reference`；v12 新契约缺失时先修复，旧库才记录 `rhythm_missing` 并回退 |
| `deconstruction-library/{书名}/剧情/情绪模块.md` | `{项目}/对标/{书名}/剧情/情绪模块.md` | 情绪模块已同步后，日更用它选择 `selected_emotion_module`；v12 新契约缺失时先修复，旧库才记录 `module_missing` 并回退 |
| `deconstruction-library/{书名}/剧情/*.md` | `{项目}/对标/{书名}/剧情/*.md` | 剧情线、故事线、散落情节等剧情资产；与权威节奏/情绪文件冲突时以后者为准 |
| `deconstruction-library/{书名}/章节/*.md`（第N章_摘要.md + 黄金三章 第1-3章_深度拆解.md）| `{项目}/对标/{书名}/章节/*.md` | 匹配章证据，含「关键信息与扩写技法」 |
| `deconstruction-library/{书名}/角色/*.md` | `{项目}/对标/{书名}/角色/*.md` | 角色功能位、关系与反应层参考 |
| `deconstruction-library/{书名}/设定/` | `{项目}/对标/{书名}/设定/` | 世界观、势力、金手指等设定约束参考 |
| `deconstruction-library/{书名}/拆文报告.md` | `{项目}/对标/{书名}/拆文报告.md` | 人类报告与旧deconstruction-library回退 |
| `deconstruction-library/{书名}/文风.md` | `{项目}/对标/{书名}/文风.md` | 日更文风召回必读 |

冲突规则：`对标/{书名}/剧情/情绪模块.md` 和 `对标/{书名}/剧情/节奏.md` 是写作侧情绪/节奏权威；`拆文报告.md`、`剧情/故事线.md` 只作为摘要投影。若摘要冲突，保留冲突说明并以权威文件为准。若检测到 v12 新契约deconstruction-library缺少任一权威文件，先重跑/修复拆文产物；只有 pre-v12 旧库可以标记 `legacy_deconstruction: true` 后降级。
