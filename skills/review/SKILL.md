...
```bash
python3 quality/audit/scripts/aigc_value.py <文本文件路径> --target 5
...
`aigc_value.py` 会给出本地自研 `自研AIGC值`(0-1 小数 + 百分比),也就是本文的最终 `AI占比`；引擎名为 `codex-local-aigc-v3`。它以朱雀四维代理分为主,叠加近年检测器常用的弱语言模型一致性、局部熵/TTR、风格计量、语义平滑、内容完整性/绕检噪声代理层,再结合既有 AI 味启发式合成。`text_signals.py` 会给出句长**突发度(CV)**、用字多样度、**AI/网文-AI 高频套路措辞密度**(按类别)、**重复长片段**、标点习惯、**本地AI味风险分**、综合AI味风险分。这些是客观可复算的证据,但**不是判决**——见下方「如何读脚本结果」。
...
本地 `aigc_value.py` 会输出 `human_anchor`。小说正文命中 `narrative_scene` 锚点时，只能对概率曲线、弱语言模型、局部熵、风格计量和分片代理做**软降权**，不能覆盖整章单段高风险；桌面 `朱雀人工事例.md` 这类技术说明文命中 `technical_expository` 锚点时，才允许使用 4.80% 的最终误判上限。无论哪类锚点，都不覆盖内容完整性、段落真重复、句子真重复、工程词泄漏等硬风险。
...
- `82.52%` 这类百分比原样记录。
- 外部检测值高于用户设置的 AI 味目标(常见为 5%)时,审核结论必须判为**不通过**,并回到降 AI 味回改。
...
- **自研AIGC值 / AI占比(0–1 / 0–100%)**:`aigc_value.py` 的核心交付门控分。v3 按三层合成:近年检测器代理层占 85%,朱雀四维综合占 10%,既有 AI 味启发式占 5%。朱雀四维仍是:突发性 30%、困惑度代理 25%、结构指纹 25%、跨段一致性 20%。它不是外部平台判定,但服务端会自动运行,不能靠报告手写绕过。
- **朱雀四维代理分**:
...
- **近年检测器代理层**:
  ...
  - 高质人工样本锚点:小说正文看句长/段长疏密、对话声口、物件/动作/感官密度、标点层次和低套路密度是否同时成立；技术说明文看领域术语、英文/缩写词、摘要式长句承载和低套路密度。小说锚点只做软校准，不能把整章风险压成通过；技术说明文人工锚点可使用 4.80% 最终误判上限。若存在脏码、真重复或工程词泄漏，锚点不会启用。
...
5. **发布/交付硬闸门**(用于当前短篇生产流程):
   - `自研AIGC值` 必须不高于目标值(默认 5%)；服务端会自动对正文运行,不是只读审核报告。
   - `AI创作度` 必须不高于目标值(默认 5%)。
   - `六维合计` 必须 ≤ 1,且任一单项不应 > 1。
   - `段落重复` 必须为 0。
   - `本地AI味风险分` 必须 ≤ 35/100。
   - 内容完整性必须通过:不得有无语义脏码、随机字串、拟声/词表绕检噪声。
   - 外部 AIGC 检测值若提供,必须不高于目标值；腾讯朱雀 `0.8252` = `82.52%`,直接判未过。
...
## 量化信号(脚本)
- 自研AIGC值 / AI占比:0.xxxx / X%;引擎 codex-local-aigc-v3;是否过闸
...
- `quality/audit/references/aigc-detection-current-notes.md` —— 近年公开 AI 文本检测逻辑与 `codex-local-aigc-v3` 本地化规则。
